...
<span id="el_prescription_details_formulation">
    <select
        ...
        data-select2-id="fprescription_detailsedit_x_formulation"
        <?php } ?>
        data-table="prescription_details"
...
<script>
loadjs.ready("fprescription_detailsedit", function() {
    var options = { name: "x_formulation", selectId: "fprescription_detailsedit_x_formulation" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprescription_detailsedit.lists.formulation?.lookupOptions.length) {
        options.data = { id: "x_formulation", form: "fprescription_detailsedit" };
    } else {
        options.ajax = { id: "x_formulation", form: "fprescription_detailsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
