...
<span id="el_patient_vaccinations_visit_id">
    <select
        ...
        data-select2-id="fpatient_vaccinationsedit_x_visit_id"
        <?php } ?>
        data-table="patient_vaccinations"
...
<script>
loadjs.ready("fpatient_vaccinationsedit", function() {
    var options = { name: "x_visit_id", selectId: "fpatient_vaccinationsedit_x_visit_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatient_vaccinationsedit.lists.visit_id?.lookupOptions.length) {
        options.data = { id: "x_visit_id", form: "fpatient_vaccinationsedit" };
    } else {
        options.ajax = { id: "x_visit_id", form: "fpatient_vaccinationsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
