...
$Page->showMessage();
?>
<form name="fpatient_queueadd" id="fpatient_queueadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_patient_queue_patient_id">
    <select
        ...
        data-select2-id="fpatient_queueadd_x_patient_id"
        <?php } ?>
...
loadjs.ready("fpatient_queueadd", function() {
    var options = { name: "x_patient_id", selectId: "fpatient_queueadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatient_queueadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fpatient_queueadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fpatient_queueadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_patient_queue_section">
    <select
        ...
        data-select2-id="fpatient_queueadd_x_section"
        <?php } ?>
        data-table="patient_queue"
...
<script>
loadjs.ready("fpatient_queueadd", function() {
    var options = { name: "x_section", selectId: "fpatient_queueadd_x_section" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatient_queueadd.lists.section?.lookupOptions.length) {
        options.data = { id: "x_section", form: "fpatient_queueadd" };
    } else {
        options.ajax = { id: "x_section", form: "fpatient_queueadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpatient_queueadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpatient_queueadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
