...
// Page object
$FullHaemogramParametersAdd = &$Page;
?>
<script>
...
var currentForm;
var ffull_haemogram_parametersadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("ffull_haemogram_parametersadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="ffull_haemogram_parametersadd" id="ffull_haemogram_parametersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_full_haemogram_parameters_test">
    <select
        ...
        data-select2-id="ffull_haemogram_parametersadd_x_test"
        <?php } ?>
...
<?php if (!$Page->test->IsNativeSelect) { ?>
<script>
loadjs.ready("ffull_haemogram_parametersadd", function() {
    var options = { name: "x_test", selectId: "ffull_haemogram_parametersadd_x_test" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffull_haemogram_parametersadd.lists.test?.lookupOptions.length) {
        options.data = { id: "x_test", form: "ffull_haemogram_parametersadd" };
    } else {
        options.ajax = { id: "x_test", form: "ffull_haemogram_parametersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffull_haemogram_parametersadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffull_haemogram_parametersadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
