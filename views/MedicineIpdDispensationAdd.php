...
// Page object
$MedicineIpdDispensationAdd = &$Page;
?>
<script>
...
var currentForm;
var fmedicine_ipd_dispensationadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmedicine_ipd_dispensationadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fmedicine_ipd_dispensationadd" id="fmedicine_ipd_dispensationadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_medicine_ipd_dispensation_status">
    <select
        ...
        data-select2-id="fmedicine_ipd_dispensationadd_x_status"
        <?php } ?>
...
loadjs.ready("fmedicine_ipd_dispensationadd", function() {
    var options = { name: "x_status", selectId: "fmedicine_ipd_dispensationadd_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmedicine_ipd_dispensationadd.lists.status?.lookupOptions.length) {
        options.data = { id: "x_status", form: "fmedicine_ipd_dispensationadd" };
    } else {
        options.ajax = { id: "x_status", form: "fmedicine_ipd_dispensationadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("medicine_ipd_dispensation_details", explode(",", $Page->getCurrentDetailTable())) && $medicine_ipd_dispensation_details->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmedicine_ipd_dispensationadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmedicine_ipd_dispensationadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
