...
<span id="el_diagnosis_lab_test_report_id">
    <select
        ...
        data-select2-id="fdiagnosisadd_x_lab_test_report_id"
        <?php } ?>
...
loadjs.ready("fdiagnosisadd", function() {
    var options = { name: "x_lab_test_report_id", selectId: "fdiagnosisadd_x_lab_test_report_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdiagnosisadd.lists.lab_test_report_id?.lookupOptions.length) {
        options.data = { id: "x_lab_test_report_id", form: "fdiagnosisadd" };
    } else {
        options.ajax = { id: "x_lab_test_report_id", form: "fdiagnosisadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
