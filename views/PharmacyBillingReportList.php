...
// Page object
$PharmacyBillingReportList = &$Page;
?>
...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fpharmacy_billing_reportsrch" id="fpharmacy_billing_reportsrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fpharmacy_billing_reportsrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { pharmacy_billing_report: currentTable } });
var currentForm;
var fpharmacy_billing_reportsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    let form = new ew.FormBuilder()
        .setId("fpharmacy_billing_reportsrch")
        .setPageId("list")
...
?>
    <div id="xs_patient_id" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->patient_id->UseFilter ? " ew-filter-field" : "" ?>">
        <select
            ...
            data-select2-id="fpharmacy_billing_reportsrch_x_patient_id"
        ...
        <script>
        loadjs.ready("fpharmacy_billing_reportsrch", function() {
            var options = {
                name: "x_patient_id",
                selectId: "fpharmacy_billing_reportsrch_x_patient_id",
                ajax: { id: "x_patient_id", form: "fpharmacy_billing_reportsrch", limit: ew.FILTER_PAGE_SIZE, data: { ajax: "filter" } }
            };
            options = Object.assign({}, ew.filterOptions, options, ew.vars.tables.pharmacy_billing_report.fields.patient_id.filterOptions);
...
<?php
if (!$Page->dispensation_type->UseFilter) {
    $Page->SearchColumnCount++;
}
?>
    <div id="xs_dispensation_type" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->dispensation_type->UseFilter ? " ew-filter-field" : "" ?>">
        <select
            id="x_dispensation_type"
            name="x_dispensation_type[]"
            class="form-control ew-select<?= $Page->dispensation_type->isInvalidClass() ?>"
            data-select2-id="fpharmacy_billing_reportsrch_x_dispensation_type"
            data-table="pharmacy_billing_report"
            data-field="x_dispensation_type"
            data-caption="<?= HtmlEncode(RemoveHtml($Page->dispensation_type->caption())) ?>"
            data-filter="true"
            multiple
            size="1"
            ...
            <?= $Page->dispensation_type->selectOptionListHtml("x_dispensation_type", true) ?>
        </select>
        <div class="invalid-feedback"><?= $Page->dispensation_type->getErrorMessage(false) ?></div>
        <script>
        loadjs.ready("fpharmacy_billing_reportsrch", function() {
            var options = {
                name: "x_dispensation_type",
                selectId: "fpharmacy_billing_reportsrch_x_dispensation_type",
                ajax: { id: "x_dispensation_type", form: "fpharmacy_billing_reportsrch", limit: ew.FILTER_PAGE_SIZE, data: { ajax: "filter" } }
            };
            options = Object.assign({}, ew.filterOptions, options, ew.vars.tables.pharmacy_billing_report.fields.dispensation_type.filterOptions);
            ew.createFilter(options);
        });
        </script>
    </div><!-- /.col-sm-auto -->
...
loadjs.ready(["fpharmacy_billing_reportsrch", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpharmacy_billing_reportsrch", "x_date_created", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</div>
        ...
        <div id="el2_pharmacy_billing_report_date_created" class="ew-search-field2">
<input type="<?= $Page->date_created->getInputTextType() ?>" name="y_date_created" id="y_date_created" data-table="pharmacy_billing_report" data-field="x_date_created" value="<?= $Page->date_created->EditValue2 ?>" placeholder="<?= HtmlEncode($Page->date_created->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->date_created->formatPattern()) ?>"<?= $Page->date_created->editAttributes() ?>>
<div class="invalid-feedback"><?= $Page->date_created->getErrorMessage(false) ?></div>
<?php if (!$Page->date_created->ReadOnly && !$Page->date_created->Disabled && !isset($Page->date_created->EditAttrs["readonly"]) && !isset($Page->date_created->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fpharmacy_billing_reportsrch", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpharmacy_billing_reportsrch", "y_date_created", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</div>
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fpharmacy_billing_reportsrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fpharmacy_billing_reportsrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fpharmacy_billing_reportsrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fpharmacy_billing_reportsrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_pharmacy_billing_report" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_pharmacy_billing_reportlist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_pharmacy_billing_report_dispensation_type" class="el_pharmacy_billing_report_dispensation_type">
