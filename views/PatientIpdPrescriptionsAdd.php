...
// Page object
$PatientIpdPrescriptionsAdd = &$Page;
?>
<script>
...
var currentForm;
var fpatient_ipd_prescriptionsadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fpatient_ipd_prescriptionsadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fpatient_ipd_prescriptionsadd" id="fpatient_ipd_prescriptionsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_patient_ipd_prescriptions_patient_id">
    <select
        ...
        data-select2-id="fpatient_ipd_prescriptionsadd_x_patient_id"
        <?php } ?>
...
<?php if (!$Page->patient_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fpatient_ipd_prescriptionsadd", function() {
    var options = { name: "x_patient_id", selectId: "fpatient_ipd_prescriptionsadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatient_ipd_prescriptionsadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fpatient_ipd_prescriptionsadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fpatient_ipd_prescriptionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("patient_ipd_prescription_details", explode(",", $Page->getCurrentDetailTable())) && $patient_ipd_prescription_details->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpatient_ipd_prescriptionsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpatient_ipd_prescriptionsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
