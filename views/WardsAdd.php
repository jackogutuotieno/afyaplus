...
<span id="el_wards_floor_id">
    <select
        ...
        data-select2-id="fwardsadd_x_floor_id"
        <?php } ?>
        data-table="wards"
...
loadjs.ready("fwardsadd", function() {
    var options = { name: "x_floor_id", selectId: "fwardsadd_x_floor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fwardsadd.lists.floor_id?.lookupOptions.length) {
        options.data = { id: "x_floor_id", form: "fwardsadd" };
    } else {
        options.ajax = { id: "x_floor_id", form: "fwardsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("beds", explode(",", $Page->getCurrentDetailTable())) && $beds->DetailAdd) {
?>
