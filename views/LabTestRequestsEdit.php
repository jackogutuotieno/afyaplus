...
<span id="el_lab_test_requests_visit_id">
    <select
        ...
        data-select2-id="flab_test_requestsedit_x_visit_id"
        <?php } ?>
        data-table="lab_test_requests"
...
<script>
loadjs.ready("flab_test_requestsedit", function() {
    var options = { name: "x_visit_id", selectId: "flab_test_requestsedit_x_visit_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flab_test_requestsedit.lists.visit_id?.lookupOptions.length) {
        options.data = { id: "x_visit_id", form: "flab_test_requestsedit" };
    } else {
        options.ajax = { id: "x_visit_id", form: "flab_test_requestsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
