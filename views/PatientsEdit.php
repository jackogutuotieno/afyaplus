...
?>
<main class="edit">
<form name="fpatientsedit" id="fpatientsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_patients_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_patients_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="patients" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fpatientsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpatientsedit", "x_date_of_birth", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fpatientsedit", "editor"], function() {
    ew.createEditor("fpatientsedit", "x_physical_address", 0, 0, <?= $Page->physical_address->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_patients_employment_status">
    <select
        ...
        data-select2-id="fpatientsedit_x_employment_status"
        <?php } ?>
        data-table="patients"
...
loadjs.ready("fpatientsedit", function() {
    var options = { name: "x_employment_status", selectId: "fpatientsedit_x_employment_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatientsedit.lists.employment_status?.lookupOptions.length) {
        options.data = { id: "x_employment_status", form: "fpatientsedit" };
    } else {
        options.ajax = { id: "x_employment_status", form: "fpatientsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_patients_religion">
    <select
        ...
        data-select2-id="fpatientsedit_x_religion"
        <?php } ?>
        data-table="patients"
...
<script>
loadjs.ready("fpatientsedit", function() {
    var options = { name: "x_religion", selectId: "fpatientsedit_x_religion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatientsedit.lists.religion?.lookupOptions.length) {
        options.data = { id: "x_religion", form: "fpatientsedit" };
    } else {
        options.ajax = { id: "x_religion", form: "fpatientsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
<?php
    if (in_array("patients_dependants", explode(",", $Page->getCurrentDetailTable())) && $patients_dependants->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpatientsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpatientsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
