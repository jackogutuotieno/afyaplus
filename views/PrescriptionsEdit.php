...
?>
<main class="edit">
<form name="fprescriptionsedit" id="fprescriptionsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { prescriptions: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fprescriptionsedit;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_prescriptions_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_prescriptions_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="prescriptions" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_prescriptions_patient_id">
    <select
        ...
        data-select2-id="fprescriptionsedit_x_patient_id"
        <?php } ?>
...
<?php if (!$Page->patient_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fprescriptionsedit", function() {
    var options = { name: "x_patient_id", selectId: "fprescriptionsedit_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprescriptionsedit.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fprescriptionsedit" };
    } else {
...
<span id="el_prescriptions_created_by_user_id">
    <select
        ...
        data-select2-id="fprescriptionsedit_x_created_by_user_id"
        <?php } ?>
        data-table="prescriptions"
...
<script>
loadjs.ready("fprescriptionsedit", function() {
    var options = { name: "x_created_by_user_id", selectId: "fprescriptionsedit_x_created_by_user_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprescriptionsedit.lists.created_by_user_id?.lookupOptions.length) {
        options.data = { id: "x_created_by_user_id", form: "fprescriptionsedit" };
    } else {
        options.ajax = { id: "x_created_by_user_id", form: "fprescriptionsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("prescription_details", explode(",", $Page->getCurrentDetailTable())) && $prescription_details->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fprescriptionsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fprescriptionsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
