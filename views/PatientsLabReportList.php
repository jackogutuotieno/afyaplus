...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fpatients_lab_reportsrch" id="fpatients_lab_reportsrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fpatients_lab_reportsrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { patients_lab_report: currentTable } });
var currentForm;
var fpatients_lab_reportsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    let form = new ew.FormBuilder()
        .setId("fpatients_lab_reportsrch")
        .setPageId("list")
...
?>
    <div id="xs_patient_name" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->patient_name->UseFilter ? " ew-filter-field" : "" ?>">
        <select
            ...
            data-select2-id="fpatients_lab_reportsrch_x_patient_name"
            data-table="patients_lab_report"
        ...
        <script>
        loadjs.ready("fpatients_lab_reportsrch", function() {
            var options = {
                name: "x_patient_name",
                selectId: "fpatients_lab_reportsrch_x_patient_name",
                ajax: { id: "x_patient_name", form: "fpatients_lab_reportsrch", limit: ew.FILTER_PAGE_SIZE, data: { ajax: "filter" } }
            };
            options = Object.assign({}, ew.filterOptions, options, ew.vars.tables.patients_lab_report.fields.patient_name.filterOptions);
...
loadjs.ready(["fpatients_lab_reportsrch", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpatients_lab_reportsrch", "x_date_created", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</div>
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fpatients_lab_reportsrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fpatients_lab_reportsrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fpatients_lab_reportsrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fpatients_lab_reportsrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_patients_lab_report" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_patients_lab_reportlist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->laboratorist->Visible) { // laboratorist ?>
        <th data-name="laboratorist" class="<?= $Page->laboratorist->headerCellClass() ?>"><div id="elh_patients_lab_report_laboratorist" class="patients_lab_report_laboratorist"><?= $Page->renderFieldHeader($Page->laboratorist) ?></div></th>
<?php } ?>
<?php if ($Page->date_created->Visible) { // date_created ?>
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_patients_lab_report_status" class="el_patients_lab_report_status">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_patients_lab_report_laboratorist" class="el_patients_lab_report_laboratorist">
