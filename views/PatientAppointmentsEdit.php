...
loadjs.ready(["fpatient_appointmentsedit", "editor"], function() {
    ew.createEditor("fpatient_appointmentsedit", "x_description", 0, 0, <?= $Page->description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_patient_appointments_doctor_id">
    <select
        ...
        data-select2-id="fpatient_appointmentsedit_x_doctor_id"
        <?php } ?>
        data-table="patient_appointments"
...
<script>
loadjs.ready("fpatient_appointmentsedit", function() {
    var options = { name: "x_doctor_id", selectId: "fpatient_appointmentsedit_x_doctor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatient_appointmentsedit.lists.doctor_id?.lookupOptions.length) {
        options.data = { id: "x_doctor_id", form: "fpatient_appointmentsedit" };
    } else {
        options.ajax = { id: "x_doctor_id", form: "fpatient_appointmentsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
