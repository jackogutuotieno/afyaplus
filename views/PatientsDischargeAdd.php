...
$Page->showMessage();
?>
<form name="fpatients_dischargeadd" id="fpatients_dischargeadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_patients_discharge_patient_id">
    <select
        ...
        data-select2-id="fpatients_dischargeadd_x_patient_id"
        <?php } ?>
...
<?php if (!$Page->patient_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fpatients_dischargeadd", function() {
    var options = { name: "x_patient_id", selectId: "fpatients_dischargeadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatients_dischargeadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fpatients_dischargeadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fpatients_dischargeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fpatients_dischargeadd", "editor"], function() {
    ew.createEditor("fpatients_dischargeadd", "x_admission_reason", 0, 0, <?= $Page->admission_reason->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fpatients_dischargeadd", "editor"], function() {
    ew.createEditor("fpatients_dischargeadd", "x_discharge_condition", 0, 0, <?= $Page->discharge_condition->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpatients_dischargeadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpatients_dischargeadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
