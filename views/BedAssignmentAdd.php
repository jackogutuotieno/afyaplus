...
$Page->showMessage();
?>
<form name="fbed_assignmentadd" id="fbed_assignmentadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    fbed_assignmentadd.createAutoSuggest(Object.assign({"id":"x_patient_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->patient_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.bed_assignment.fields.patient_id.autoSuggestOptions));
});
</script>
...
<span id="el_bed_assignment_floor_id">
    <select
        ...
        data-select2-id="fbed_assignmentadd_x_floor_id"
        <?php } ?>
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    var options = { name: "x_floor_id", selectId: "fbed_assignmentadd_x_floor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbed_assignmentadd.lists.floor_id?.lookupOptions.length) {
        options.data = { id: "x_floor_id", form: "fbed_assignmentadd" };
    } else {
        options.ajax = { id: "x_floor_id", form: "fbed_assignmentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_bed_assignment_ward_type_id">
    <select
        ...
        data-select2-id="fbed_assignmentadd_x_ward_type_id"
        <?php } ?>
        data-table="bed_assignment"
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    var options = { name: "x_ward_type_id", selectId: "fbed_assignmentadd_x_ward_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbed_assignmentadd.lists.ward_type_id?.lookupOptions.length) {
        options.data = { id: "x_ward_type_id", form: "fbed_assignmentadd" };
    } else {
        options.ajax = { id: "x_ward_type_id", form: "fbed_assignmentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_bed_assignment_ward_id">
    <select
        ...
        data-select2-id="fbed_assignmentadd_x_ward_id"
        <?php } ?>
        data-table="bed_assignment"
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    var options = { name: "x_ward_id", selectId: "fbed_assignmentadd_x_ward_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbed_assignmentadd.lists.ward_id?.lookupOptions.length) {
        options.data = { id: "x_ward_id", form: "fbed_assignmentadd" };
    } else {
        options.ajax = { id: "x_ward_id", form: "fbed_assignmentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_bed_assignment_bed_id">
    <select
        ...
        data-select2-id="fbed_assignmentadd_x_bed_id"
        <?php } ?>
        data-table="bed_assignment"
        data-field="x_bed_id"
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    var options = { name: "x_bed_id", selectId: "fbed_assignmentadd_x_bed_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbed_assignmentadd.lists.bed_id?.lookupOptions.length) {
        options.data = { id: "x_bed_id", form: "fbed_assignmentadd" };
    } else {
        options.ajax = { id: "x_bed_id", form: "fbed_assignmentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_bed_assignment_status">
    <select
        ...
        data-select2-id="fbed_assignmentadd_x_status"
        <?php } ?>
        data-table="bed_assignment"
        data-field="x_status"
...
<script>
loadjs.ready("fbed_assignmentadd", function() {
    var options = { name: "x_status", selectId: "fbed_assignmentadd_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbed_assignmentadd.lists.status?.lookupOptions.length) {
        options.data = { id: "x_status", form: "fbed_assignmentadd" };
    } else {
        options.ajax = { id: "x_status", form: "fbed_assignmentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fbed_assignmentadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fbed_assignmentadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
