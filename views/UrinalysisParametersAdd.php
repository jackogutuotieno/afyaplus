...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("furinalysis_parametersadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="furinalysis_parametersadd" id="furinalysis_parametersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_urinalysis_parameters_parameter">
    <select
        ...
        data-select2-id="furinalysis_parametersadd_x_parameter"
        <?php } ?>
...
<?php if (!$Page->parameter->IsNativeSelect) { ?>
<script>
loadjs.ready("furinalysis_parametersadd", function() {
    var options = { name: "x_parameter", selectId: "furinalysis_parametersadd_x_parameter" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (furinalysis_parametersadd.lists.parameter?.lookupOptions.length) {
        options.data = { id: "x_parameter", form: "furinalysis_parametersadd" };
    } else {
        options.ajax = { id: "x_parameter", form: "furinalysis_parametersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="furinalysis_parametersadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="furinalysis_parametersadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
