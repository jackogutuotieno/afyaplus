...
// Page object
$MedicineDispensationDetailsEdit = &$Page;
?>
...
?>
<main class="edit">
<form name="fmedicine_dispensation_detailsedit" id="fmedicine_dispensation_detailsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { medicine_dispensation_details: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fmedicine_dispensation_detailsedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmedicine_dispensation_detailsedit")
        .setPageId("edit")
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_medicine_dispensation_details_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_medicine_dispensation_details_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="medicine_dispensation_details" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_medicine_dispensation_details_medicine_stock_id">
    <select
        ...
        data-select2-id="fmedicine_dispensation_detailsedit_x_medicine_stock_id"
        <?php } ?>
...
<?php if (!$Page->medicine_stock_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fmedicine_dispensation_detailsedit", function() {
    var options = { name: "x_medicine_stock_id", selectId: "fmedicine_dispensation_detailsedit_x_medicine_stock_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmedicine_dispensation_detailsedit.lists.medicine_stock_id?.lookupOptions.length) {
        options.data = { id: "x_medicine_stock_id", form: "fmedicine_dispensation_detailsedit" };
    } else {
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmedicine_dispensation_detailsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmedicine_dispensation_detailsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
