...
<span id="el_patients_county_id">
    <select
        ...
        data-select2-id="fpatientsadd_x_county_id"
        <?php } ?>
        data-table="patients"
...
loadjs.ready("fpatientsadd", function() {
    var options = { name: "x_county_id", selectId: "fpatientsadd_x_county_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatientsadd.lists.county_id?.lookupOptions.length) {
        options.data = { id: "x_county_id", form: "fpatientsadd" };
    } else {
        options.ajax = { id: "x_county_id", form: "fpatientsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_patients_subcounty_id">
    <select
        ...
        data-select2-id="fpatientsadd_x_subcounty_id"
        <?php } ?>
        data-table="patients"
...
<script>
loadjs.ready("fpatientsadd", function() {
    var options = { name: "x_subcounty_id", selectId: "fpatientsadd_x_subcounty_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpatientsadd.lists.subcounty_id?.lookupOptions.length) {
        options.data = { id: "x_subcounty_id", form: "fpatientsadd" };
    } else {
        options.ajax = { id: "x_subcounty_id", form: "fpatientsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
