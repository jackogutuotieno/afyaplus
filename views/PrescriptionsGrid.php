...
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_prescriptions_patient_id" class="el_prescriptions_patient_id">
    <select
        ...
        data-select2-id="fprescriptionsgrid_x<?= $Grid->RowIndex ?>_patient_id"
        <?php } ?>
...
loadjs.ready("fprescriptionsgrid", function() {
    var options = { name: "x<?= $Grid->RowIndex ?>_patient_id", selectId: "fprescriptionsgrid_x<?= $Grid->RowIndex ?>_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprescriptionsgrid.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x<?= $Grid->RowIndex ?>_patient_id", form: "fprescriptionsgrid" };
    } else {
