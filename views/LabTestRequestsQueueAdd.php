...
<span id="el_lab_test_requests_queue_waiting_interval">
    <select
        ...
        data-select2-id="flab_test_requests_queueadd_x_waiting_interval"
        <?php } ?>
        data-table="lab_test_requests_queue"
...
<?php if (!$Page->waiting_interval->IsNativeSelect) { ?>
<script>
loadjs.ready("flab_test_requests_queueadd", function() {
    var options = { name: "x_waiting_interval", selectId: "flab_test_requests_queueadd_x_waiting_interval" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flab_test_requests_queueadd.lists.waiting_interval?.lookupOptions.length) {
        options.data = { id: "x_waiting_interval", form: "flab_test_requests_queueadd" };
    } else {
        options.ajax = { id: "x_waiting_interval", form: "flab_test_requests_queueadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_lab_test_requests_queue_status">
    <select
        ...
        data-select2-id="flab_test_requests_queueadd_x_status"
        <?php } ?>
        data-table="lab_test_requests_queue"
        data-field="x_status"
...
<script>
loadjs.ready("flab_test_requests_queueadd", function() {
    var options = { name: "x_status", selectId: "flab_test_requests_queueadd_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flab_test_requests_queueadd.lists.status?.lookupOptions.length) {
        options.data = { id: "x_status", form: "flab_test_requests_queueadd" };
    } else {
        options.ajax = { id: "x_status", form: "flab_test_requests_queueadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
