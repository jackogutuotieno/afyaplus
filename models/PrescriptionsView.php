 ...
 * Page class
 */
class PrescriptionsView extends Prescriptions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PrescriptionsView";
    ...
    // CSS class/style
    public $CurrentPageName = "prescriptionsview";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "prescriptionsview"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "prescriptionslist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "prescriptionslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_prescription_details"
        $item = &$option->add("detail_prescription_details");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("prescription_details", "TblCaption");
        $detailTbl = Container("prescription_details");
        ...
        if (!$this->ShowMultipleDetails) { // Skip record count if show multiple details
            $body .= "&nbsp;" . str_replace(["%c", "%s"], [Container("prescription_details")->Count, "red"], $Language->phrase("DetailCount"));
        }
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("prescriptiondetailslist?" . Config("TABLE_SHOW_MASTER") . "=prescriptions&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("PrescriptionDetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'prescriptions')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=prescription_details"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "prescription_details";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'prescriptions')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=prescription_details"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'prescription_details');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "prescription_details";
        }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (prescription_details)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("prescription_details", explode(",", $this->getCurrentDetailTable()))) {
            $prescription_details = new PrescriptionDetailsList();
            $rsdetail = $prescription_details->loadRs($prescription_details->getDetailFilterFromSession(), $prescription_details->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($prescription_details);
                    $prescription_details->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("prescription_details", $detailTblVar)) {
                $detailPageObj = Container("PrescriptionDetailsGrid");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("prescriptionslist"), "", $this->TableVar, true);
        $pageId = "view";
