...
/**
 * Table class for medicine_dispensation_details
 */
class MedicineDispensationDetails extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $id;
    public $medicine_dispensation_id;
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "medicine_dispensation_details";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->medicine_dispensation_id = new DbField(
            $this, // Table
            'x_medicine_dispensation_id', // Variable name
            'medicine_dispensation_id', // Name
            '`medicine_dispensation_id`', // Expression
            '`medicine_dispensation_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`medicine_dispensation_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->medicine_dispensation_id->InputTextType = "text";
        $this->medicine_dispensation_id->Raw = true;
        $this->medicine_dispensation_id->IsForeignKey = true; // Foreign key field
        $this->medicine_dispensation_id->Nullable = false; // NOT NULL field
        $this->medicine_dispensation_id->Required = true; // Required field
        $this->medicine_dispensation_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->medicine_dispensation_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['medicine_dispensation_id'] = &$this->medicine_dispensation_id;
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "medicine_dispensation") {
            ...
            if ($this->medicine_dispensation_id->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->medicine_dispensation_id->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "medicine_dispensation") {
            $masterTable = Container("medicine_dispensation");
            if ($this->medicine_dispensation_id->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->medicine_dispensation_id, $this->medicine_dispensation_id->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "medicine_dispensation":
                $key = $keys["medicine_dispensation_id"] ?? "";
                if (EmptyValue($key)) {
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["medicine_dispensation_id"], $this->medicine_dispensation_id->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "medicine_dispensation":
                return GetKeyFilter($this->medicine_dispensation_id, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "medicine_dispensation_details";
    }
    ...
    public function applyUserIDFilters($filter, $id = "")
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            if ($this->getCurrentMasterTable() == "medicine_dispensation" || $this->getCurrentMasterTable() == "") {
                $filter = $this->addDetailUserIDFilter($filter, "medicine_dispensation"); // Add detail User ID filter
            }
        }
        return $filter;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->medicine_dispensation_id->DbValue = $row['medicine_dispensation_id'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("medicinedispensationdetailslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "medicinedispensationdetailsview" => $Language->phrase("View"),
            "medicinedispensationdetailsedit" => $Language->phrase("Edit"),
            "medicinedispensationdetailsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "medicinedispensationdetailslist";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "MedicineDispensationDetailsView",
            Config("API_ADD_ACTION") => "MedicineDispensationDetailsAdd",
            Config("API_EDIT_ACTION") => "MedicineDispensationDetailsEdit",
            Config("API_DELETE_ACTION") => "MedicineDispensationDetailsDelete",
            Config("API_LIST_ACTION") => "MedicineDispensationDetailsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("medicinedispensationdetailsview", $parm);
        } else {
            $url = $this->keyUrl("medicinedispensationdetailsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "medicinedispensationdetailsadd?" . $parm;
        } else {
            $url = "medicinedispensationdetailsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationdetailsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationdetailslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationdetailsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationdetailslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("medicinedispensationdetailsdelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "medicine_dispensation" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->medicine_dispensation_id->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    {
        global $Response;
        $listPage = "MedicineDispensationDetailsList";
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "medicine_dispensation") {
            $filterWrk = Container("medicine_dispensation")->addUserIDFilter($filterWrk);
        }
        return $filterWrk;
    }
    ...
    public function addDetailUserIDFilter($filter, $currentMasterTable)
    {
        ...
        if ($currentMasterTable == "medicine_dispensation") {
            ...
            if (!$mastertable->userIDAllow()) {
                $subqueryWrk = $mastertable->getUserIDSubquery($this->medicine_dispensation_id, $mastertable->id);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'medicine_dispensation_details');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'medicine_dispensation_details', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'medicine_dispensation_details', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'medicine_dispensation_details', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    {
        global $Language;
        $table = 'medicine_dispensation_details';
