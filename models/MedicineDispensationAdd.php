 ...
 * Page class
 */
class MedicineDispensationAdd extends MedicineDispensation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MedicineDispensationAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "medicinedispensationadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("medicinedispensationlist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "medicinedispensationlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "medicinedispensationview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "medicinedispensationlist") {
                            ...
                            $returnUrl = "medicinedispensationlist"; // Return list page content
                        }
                    }
    ...
    public function renderRow()
    {
        ...
        // dispensation_type
        $this->dispensation_type->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("medicine_dispensation_details", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("MedicineDispensationDetailsGrid");
            if (in_array("medicine_dispensation_details", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->medicine_dispensation_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "medicine_dispensation_details"); // Load user level of detail table
