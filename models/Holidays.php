...
/**
 * Table class for holidays
 */
class Holidays extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // holiday_name
        $this->holiday_name = new DbField(
            $this, // Table
            'x_holiday_name', // Variable name
            'holiday_name', // Name
            '`holiday_name`', // Expression
            '`holiday_name`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`holiday_name`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->holiday_name->InputTextType = "text";
        $this->holiday_name->Nullable = false; // NOT NULL field
        $this->holiday_name->Required = true; // Required field
        $this->holiday_name->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['holiday_name'] = &$this->holiday_name;
        ...
        $this->date = new DbField(
            ...
            'date', // Name
            '`date`', // Expression
            CastDateFieldForLike("`date`", 7, "DB"), // Basic search expression
            133, // Type
            10, // Size
            7, // Date/Time format
            false, // Is upload field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "holidays";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->holiday_name->DbValue = $row['holiday_name'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("holidayslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "holidaysview" => $Language->phrase("View"),
            "holidaysedit" => $Language->phrase("Edit"),
            "holidaysadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "HolidaysView",
            Config("API_ADD_ACTION") => "HolidaysAdd",
            Config("API_EDIT_ACTION") => "HolidaysEdit",
            Config("API_DELETE_ACTION") => "HolidaysDelete",
            Config("API_LIST_ACTION") => "HolidaysList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("holidaysview", $parm);
        } else {
            $url = $this->keyUrl("holidaysview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("holidaysedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("holidayslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("holidaysadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("holidayslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("holidaysdelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // holiday_name
        $this->holiday_name->setupEditAttributes();
        if (!$this->holiday_name->Raw) {
            $this->holiday_name->CurrentValue = HtmlDecode($this->holiday_name->CurrentValue);
        }
        $this->holiday_name->EditValue = $this->holiday_name->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->holiday_name);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->holiday_name);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'holidays');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'holidays', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'holidays', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'holidays', $fldname, $key, $oldvalue);
            }
        }
    }
