 ...
 * Page class
 */
class MedicineIpdDispensationAdd extends MedicineIpdDispensation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MedicineIpdDispensationAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "medicineipddispensationadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("medicineipddispensationlist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "medicineipddispensationlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "medicineipddispensationview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "medicineipddispensationlist") {
                            ...
                            $returnUrl = "medicineipddispensationlist"; // Return list page content
                        }
                    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("medicine_ipd_dispensation_details", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("MedicineIpdDispensationDetailsGrid");
            if (in_array("medicine_ipd_dispensation_details", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->medicine_dispensation_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "medicine_ipd_dispensation_details"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->medicine_dispensation_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
