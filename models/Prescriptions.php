...
class Prescriptions extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->patient_id->Lookup = new Lookup($this->patient_id, 'patients', false, 'id', ["patient_name","id","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(first_name,' ',last_name,'" . ValueSeparator(1, $this->patient_id) . "',COALESCE(`id`,''))");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "prescription_details") {
            $detailUrl = Container("prescription_details")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "prescriptionslist";
        }
        return $detailUrl;
    }
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("prescriptionsedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("prescriptionsadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'prescriptions');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'prescriptions', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'prescriptions', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'prescriptions', $fldname, $key, $oldvalue);
            }
        }
    }
