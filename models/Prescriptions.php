...
/**
 * Table class for prescriptions
 */
class Prescriptions extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "prescriptions";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // lab_report_id
        $this->lab_report_id = new DbField(
            $this, // Table
            'x_lab_report_id', // Variable name
            'lab_report_id', // Name
            '`lab_report_id`', // Expression
            '`lab_report_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`lab_report_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->lab_report_id->InputTextType = "text";
        $this->lab_report_id->Raw = true;
        $this->lab_report_id->Nullable = false; // NOT NULL field
        $this->lab_report_id->Required = true; // Required field
        $this->lab_report_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->lab_report_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['lab_report_id'] = &$this->lab_report_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "prescriptions";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->lab_report_id->DbValue = $row['lab_report_id'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("prescriptionslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "prescriptionsview" => $Language->phrase("View"),
            "prescriptionsedit" => $Language->phrase("Edit"),
            "prescriptionsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PrescriptionsView",
            Config("API_ADD_ACTION") => "PrescriptionsAdd",
            Config("API_EDIT_ACTION") => "PrescriptionsEdit",
            Config("API_DELETE_ACTION") => "PrescriptionsDelete",
            Config("API_LIST_ACTION") => "PrescriptionsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("prescriptionsview", $parm);
        } else {
            $url = $this->keyUrl("prescriptionsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "prescriptionsadd?" . $parm;
        } else {
            $url = "prescriptionsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prescriptionsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prescriptionslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prescriptionsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prescriptionslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("prescriptionsdelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->lab_report_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->lab_report_id);
    ...
    {
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM prescriptions";
