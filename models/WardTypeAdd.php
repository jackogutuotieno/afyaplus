 ...
 * Page class
 */
class WardTypeAdd extends WardType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WardTypeAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "wardtypeadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("wardtypelist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "wardtypelist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "wardtypeview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "wardtypelist") {
                            ...
                            $returnUrl = "wardtypelist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ward_type' first before field var 'x_ward_type'
        $val = $CurrentForm->hasValue("ward_type") ? $CurrentForm->getValue("ward_type") : $CurrentForm->getValue("x_ward_type");
        if (!$this->ward_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ward_type->Visible = false; // Disable update for API request
            } else {
                $this->ward_type->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->ward_type->CurrentValue = $this->ward_type->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        // ward_type
        $this->ward_type->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->ward_type->EditValue = HtmlEncode($this->ward_type->CurrentValue);
        ...
        $validateForm = true;
            ...
            if ($this->ward_type->Visible && $this->ward_type->Required) {
                if (!$this->ward_type->IsDetailKey && EmptyValue($this->ward_type->FormValue)) {
                    $this->ward_type->addErrorMessage(str_replace("%s", $this->ward_type->caption(), $this->ward_type->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // ward_type
        $this->ward_type->setDbValueDef($rsnew, $this->ward_type->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['ward_type'])) { // ward_type
            $this->ward_type->setFormValue($row['ward_type']);
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("wardtypelist"), "", $this->TableVar, true);
