 ...
 * Page class
 */
class LabTestRequestsQueueAdd extends LabTestRequestsQueue
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LabTestRequestsQueueAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "labtestrequestsqueueadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("labtestrequestsqueuelist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "labtestrequestsqueueview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "labtestrequestsqueuelist") {
                            ...
                            $returnUrl = "labtestrequestsqueuelist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->waiting_time->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->waiting_time->Visible = false; // Disable update for API request
            } else {
                $this->waiting_time->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'waiting_interval' first before field var 'x_waiting_interval'
        $val = $CurrentForm->hasValue("waiting_interval") ? $CurrentForm->getValue("waiting_interval") : $CurrentForm->getValue("x_waiting_interval");
        if (!$this->waiting_interval->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->waiting_interval->Visible = false; // Disable update for API request
            } else {
                $this->waiting_interval->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        $this->lab_test_requests_details_id->RowCssClass = "row";
        ...
        // waiting_interval
        $this->waiting_interval->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->waiting_time->FormValue)) {
                $this->waiting_time->addErrorMessage($this->waiting_time->getErrorMessage(false));
            }
            if ($this->waiting_interval->Visible && $this->waiting_interval->Required) {
                if (!$this->waiting_interval->IsDetailKey && EmptyValue($this->waiting_interval->FormValue)) {
                    $this->waiting_interval->addErrorMessage(str_replace("%s", $this->waiting_interval->caption(), $this->waiting_interval->RequiredErrorMessage));
                }
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            if (in_array("lab_test_reports", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->lab_test_requests_queue_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "lab_test_reports"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->lab_test_requests_queue_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // waiting_time
        $this->waiting_time->setDbValueDef($rsnew, $this->waiting_time->CurrentValue, false);

        // waiting_interval
        $this->waiting_interval->setDbValueDef($rsnew, $this->waiting_interval->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['waiting_time'])) { // waiting_time
            $this->waiting_time->setFormValue($row['waiting_time']);
        }
        if (isset($row['waiting_interval'])) { // waiting_interval
            $this->waiting_interval->setFormValue($row['waiting_interval']);
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("lab_test_reports", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailAdd) {
                    ...
                    $detailPageObj->lab_test_requests_queue_id->IsDetailKey = true;
                    $detailPageObj->lab_test_requests_queue_id->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->lab_test_requests_queue_id->setSessionValue($detailPageObj->lab_test_requests_queue_id->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("labtestrequestsqueuelist"), "", $this->TableVar, true);
