 ...
 * Page class
 */
class ProcurementOverview extends ReportTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProcurementOverview";
    ...
    public $CurrentPageName = "procurementoverview";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Procurement_Overview';
        $this->TableName = 'Procurement Overview';
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Procurement Overview');
        }
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("ProcurementReportSummary");
                $Page->run();
                $Page->ProcurementbyMonth->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ProcurementbyMonth->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("ProcurementReportSummary");
                $Page->run();
                $Page->ProcurementbyYear->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ProcurementbyYear->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 3) {
                $Page = Container("ProcurementReportSummary");
                $Page->run();
                $Page->ProcurementbySupplier->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ProcurementbySupplier->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 4) {
                $Page = Container("ProcurementReportSummary");
                $Page->run();
                $Page->ProcurementbyCategory->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ProcurementbyCategory->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 5) {
                $Page = Container("ProcurementReportSummary");
                $Page->run();
                $Page->ProcurementbySubcategory->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ProcurementbySubcategory->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
