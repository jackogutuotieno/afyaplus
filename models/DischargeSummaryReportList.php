 ...
 * Page class
 */
class DischargeSummaryReportList extends DischargeSummaryReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DischargeSummaryReportList";
    ...
    public $FormName = "fdischarge_summary_reportlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "dischargesummaryreportlist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'discharge_summary_report';
        $this->TableName = 'discharge_summary_report';
        ...
        // Table object (discharge_summary_report)
        if (!isset($GLOBALS["discharge_summary_report"]) || $GLOBALS["discharge_summary_report"]::class == PROJECT_NAMESPACE . "discharge_summary_report") {
            $GLOBALS["discharge_summary_report"] = &$this;
        }
        ...
        $this->MultiDeleteUrl = "dischargesummaryreportdelete";
        $this->MultiUpdateUrl = "dischargesummaryreportupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'discharge_summary_report');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "dischargesummaryreportview"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fdischarge_summary_reportgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->discharge_date->AdvancedSearch->toJson(), ","); // Field discharge_date
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdischarge_summary_reportsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->admission_reason->AdvancedSearch->SearchOperator = @$filter["z_admission_reason"];
        $this->admission_reason->AdvancedSearch->SearchCondition = @$filter["v_admission_reason"];
        $this->admission_reason->AdvancedSearch->SearchValue2 = @$filter["y_admission_reason"];
        $this->admission_reason->AdvancedSearch->SearchOperator2 = @$filter["w_admission_reason"];
        $this->admission_reason->AdvancedSearch->save();

        // Field discharge_condition
        $this->discharge_condition->AdvancedSearch->SearchValue = @$filter["x_discharge_condition"];
        $this->discharge_condition->AdvancedSearch->SearchOperator = @$filter["z_discharge_condition"];
        $this->discharge_condition->AdvancedSearch->SearchCondition = @$filter["v_discharge_condition"];
        $this->discharge_condition->AdvancedSearch->SearchValue2 = @$filter["y_discharge_condition"];
        $this->discharge_condition->AdvancedSearch->SearchOperator2 = @$filter["w_discharge_condition"];
        $this->discharge_condition->AdvancedSearch->save();
        ...
        $this->discharge_date->AdvancedSearch->SearchValue = @$filter["x_discharge_date"];
        $this->discharge_date->AdvancedSearch->SearchOperator = @$filter["z_discharge_date"];
        $this->discharge_date->AdvancedSearch->SearchCondition = @$filter["v_discharge_date"];
        $this->discharge_date->AdvancedSearch->SearchValue2 = @$filter["y_discharge_date"];
        $this->discharge_date->AdvancedSearch->SearchOperator2 = @$filter["w_discharge_date"];
        $this->discharge_date->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->admission_reason, $default, false); // admission_reason
        $this->buildSearchSql($where, $this->discharge_condition, $default, false); // discharge_condition
        ...
        $this->buildSearchSql($where, $this->discharge_date, $default, false); // discharge_date
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->admission_reason->AdvancedSearch->save(); // admission_reason
            $this->discharge_condition->AdvancedSearch->save(); // discharge_condition
            $this->created_by_user_id->AdvancedSearch->save(); // created_by_user_id
            $this->admission_date->AdvancedSearch->save(); // admission_date
            $this->discharge_date->AdvancedSearch->save(); // discharge_date
    ...
    public function showFilterList()
    {
        ...
        // Field admission_date
        $filter = $this->queryBuilderWhere("admission_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->admission_date, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->admission_date->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field discharge_date
        $filter = $this->queryBuilderWhere("discharge_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->discharge_date, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->discharge_date->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->admission_reason->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->discharge_condition->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->discharge_date->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->admission_reason->AdvancedSearch->unsetSession();
        $this->discharge_condition->AdvancedSearch->unsetSession();
        $this->created_by_user_id->AdvancedSearch->unsetSession();
        $this->admission_date->AdvancedSearch->unsetSession();
        $this->discharge_date->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->admission_reason->AdvancedSearch->load();
        $this->discharge_condition->AdvancedSearch->load();
        $this->created_by_user_id->AdvancedSearch->load();
        $this->admission_date->AdvancedSearch->load();
        $this->discharge_date->AdvancedSearch->load();
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"discharge_summary_report\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdischarge_summary_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdischarge_summary_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdischarge_summary_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdischarge_summary_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdischarge_summary_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_discharge_summary_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_discharge_summary_report",
    ...
    protected function loadSearchValues()
    {
        ...
        // admission_reason
        if ($this->admission_reason->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->admission_reason->AdvancedSearch->SearchValue != "" || $this->admission_reason->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // discharge_condition
        if ($this->discharge_condition->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->discharge_condition->AdvancedSearch->SearchValue != "" || $this->discharge_condition->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // admission_date
        if ($this->admission_date->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // discharge_date
        if ($this->discharge_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->discharge_date->AdvancedSearch->SearchValue != "" || $this->discharge_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->discharge_date->TooltipValue = "";
            ...
            $this->admission_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->admission_date->AdvancedSearch->SearchValue, $this->admission_date->formatPattern()), $this->admission_date->formatPattern()));
            $this->admission_date->PlaceHolder = RemoveHtml($this->admission_date->caption());

            // discharge_date
            $this->discharge_date->setupEditAttributes();
            $this->discharge_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->discharge_date->AdvancedSearch->SearchValue, $this->discharge_date->formatPattern()), $this->discharge_date->formatPattern()));
