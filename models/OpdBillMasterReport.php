...
/**
 * Table class for opd_bill_master_report
 */
class OpdBillMasterReport extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "opd_bill_master_report";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // uhid
        $this->uhid = new DbField(
            $this, // Table
            'x_uhid', // Variable name
            'uhid', // Name
            '`uhid`', // Expression
            '`uhid`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`uhid`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->uhid->InputTextType = "text";
        $this->uhid->Raw = true;
        $this->uhid->IsAutoIncrement = true; // Autoincrement field
        $this->uhid->Nullable = false; // NOT NULL field
        $this->uhid->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->uhid->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['uhid'] = &$this->uhid;
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "opd_consultation") {
            $detailUrl = Container("opd_consultation")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "opd_lab_master_bill") {
        ...
        if ($detailUrl == "") {
            $detailUrl = "opdbillmasterreportlist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "opd_bill_master_report";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            ...
            $this->uhid->setDbValue($conn->lastInsertId());
            $rs['uhid'] = $this->uhid->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        // Return auto increment field
        if ($success) {
            if (!isset($rs['uhid']) && !EmptyValue($this->uhid->CurrentValue)) {
                $rs['uhid'] = $this->uhid->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("opdbillmasterreportlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "opdbillmasterreportview" => $Language->phrase("View"),
            "opdbillmasterreportedit" => $Language->phrase("Edit"),
            "opdbillmasterreportadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "opdbillmasterreportlist";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "OpdBillMasterReportView",
            Config("API_ADD_ACTION") => "OpdBillMasterReportAdd",
            Config("API_EDIT_ACTION") => "OpdBillMasterReportEdit",
            Config("API_DELETE_ACTION") => "OpdBillMasterReportDelete",
            Config("API_LIST_ACTION") => "OpdBillMasterReportList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("opdbillmasterreportview", $parm);
        } else {
            $url = $this->keyUrl("opdbillmasterreportview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "opdbillmasterreportadd?" . $parm;
        } else {
            $url = "opdbillmasterreportadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("opdbillmasterreportedit", $parm);
        } else {
            $url = $this->keyUrl("opdbillmasterreportedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("opdbillmasterreportlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("opdbillmasterreportadd", $parm);
        } else {
            $url = $this->keyUrl("opdbillmasterreportadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("opdbillmasterreportlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("opdbillmasterreportdelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "OpdBillMasterReportList";
    ...
    public function renderEditRow()
    {
        ...
        // uhid
        $this->uhid->setupEditAttributes();
        $this->uhid->EditValue = $this->uhid->CurrentValue;
        $this->uhid->PlaceHolder = RemoveHtml($this->uhid->caption());
        if (strval($this->uhid->EditValue) != "" && is_numeric($this->uhid->EditValue)) {
            $this->uhid->EditValue = $this->uhid->EditValue;
        }
    ...
    public function rowRendered()
    {
        ...
        if ($this->visit_date->CurrentValue > $current_date) {
            $this->status->CellAttrs["style"] = "background-color: #15b20b; color: white";
            $this->status->ViewValue = "New Bill"; 
        } else if ($this->visit_date->CurrentValue < $current_date) {
