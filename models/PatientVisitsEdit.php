 ...
 * Page class
 */
class PatientVisitsEdit extends PatientVisits
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientVisitsEdit";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_queue", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
        ...
        if (in_array("cash_payments", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        ...
        if (in_array("radiology_billing_report", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PharmacyBillingReportGrid");
        if (in_array("pharmacy_billing_report", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientsLabReportGrid");
        if (in_array("patients_lab_report", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MedicineDispensationGrid");
        if (in_array("medicine_dispensation", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientsRadiologyReportsGrid");
        if (in_array("patients_radiology_reports", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PatientQueueGrid");
            if (in_array("patient_queue", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
            ...
            $detailPage = Container("CashPaymentsGrid");
            if (in_array("cash_payments", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
            ...
            $detailPage = Container("RadiologyBillingReportGrid");
            if (in_array("radiology_billing_report", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "radiology_billing_report"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("PharmacyBillingReportGrid");
            if (in_array("pharmacy_billing_report", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "pharmacy_billing_report"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("PatientsLabReportGrid");
            if (in_array("patients_lab_report", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "patients_lab_report"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("MedicineDispensationGrid");
            if (in_array("medicine_dispensation", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "medicine_dispensation"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("PatientsRadiologyReportsGrid");
            if (in_array("patients_radiology_reports", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "patients_radiology_reports"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
