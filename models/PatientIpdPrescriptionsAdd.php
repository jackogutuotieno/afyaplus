 ...
 * Page class
 */
class PatientIpdPrescriptionsAdd extends PatientIpdPrescriptions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientIpdPrescriptionsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "patientipdprescriptionsadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("patientipdprescriptionslist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "patientipdprescriptionslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "patientipdprescriptionsview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "patientipdprescriptionslist") {
                            ...
                            $returnUrl = "patientipdprescriptionslist"; // Return list page content
                        }
                    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_ipd_prescription_details", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PatientIpdPrescriptionDetailsGrid");
            if (in_array("patient_ipd_prescription_details", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->prescription_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "patient_ipd_prescription_details"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->prescription_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
