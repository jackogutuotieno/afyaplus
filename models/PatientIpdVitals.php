...
/**
 * Table class for patient_ipd_vitals
 */
class PatientIpdVitals extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "patient_ipd_vitals";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "patients_discharge") {
            $masterTable = Container("patients_discharge");
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "patients_discharge":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->admission_id, $keys["admission_id"], $this->admission_id->DataType, $this->Dbid) . " AND " . GetKeyFilter($masterTable->patient_id, $keys["patient_id"], $this->patient_id->DataType, $this->Dbid);
                }
                break;
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "patients_discharge":
                return GetKeyFilter($this->admission_id, $masterTable->admission_id->DbValue, $masterTable->admission_id->DataType, $masterTable->Dbid) . " AND " . GetKeyFilter($this->patient_id, $masterTable->patient_id->DbValue, $masterTable->patient_id->DataType, $masterTable->Dbid);
            case "patient_admissions":
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "patient_ipd_vitals";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("patientipdvitalslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "patientipdvitalsview" => $Language->phrase("View"),
            "patientipdvitalsedit" => $Language->phrase("Edit"),
            "patientipdvitalsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PatientIpdVitalsView",
            Config("API_ADD_ACTION") => "PatientIpdVitalsAdd",
            Config("API_EDIT_ACTION") => "PatientIpdVitalsEdit",
            Config("API_DELETE_ACTION") => "PatientIpdVitalsDelete",
            Config("API_LIST_ACTION") => "PatientIpdVitalsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("patientipdvitalsview", $parm);
        } else {
            $url = $this->keyUrl("patientipdvitalsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "patientipdvitalsadd?" . $parm;
        } else {
            $url = "patientipdvitalsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientipdvitalsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientipdvitalslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientipdvitalsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientipdvitalslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("patientipdvitalsdelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "patients_discharge" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
