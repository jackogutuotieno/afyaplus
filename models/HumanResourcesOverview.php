 ...
 * Page class
 */
class HumanResourcesOverview extends ReportTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HumanResourcesOverview";
    ...
    public $CurrentPageName = "humanresourcesoverview";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Human_Resources_Overview';
        $this->TableName = 'Human Resources Overview';
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Human Resources Overview');
        }
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("EmployeesReportSummary");
                $Page->run();
                $Page->EmployeesByGender->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EmployeesByGender->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("EmployeesReportSummary");
                $Page->run();
                $Page->EmployeesbyDepartment->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EmployeesbyDepartment->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 3) {
                $Page = Container("EmployeesReportSummary");
                $Page->run();
                $Page->EmployeesbyDesignation->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EmployeesbyDesignation->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
