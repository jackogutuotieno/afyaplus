 ...
 * Page class
 */
class PatientsRadiologyReportsList extends PatientsRadiologyReports
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientsRadiologyReportsList";
    ...
    public $FormName = "fpatients_radiology_reportslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "patientsradiologyreportslist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'patients_radiology_reports';
        $this->TableName = 'patients_radiology_reports';
        ...
        // Table object (patients_radiology_reports)
        if (!isset($GLOBALS["patients_radiology_reports"]) || $GLOBALS["patients_radiology_reports"]::class == PROJECT_NAMESPACE . "patients_radiology_reports") {
            $GLOBALS["patients_radiology_reports"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "patientsradiologyreportsadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "patientsradiologyreportsdelete";
        $this->MultiUpdateUrl = "patientsradiologyreportsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'patients_radiology_reports');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "patientsradiologyreportsview"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->radiologist->AdvancedSearch->toJson(), ","); // Field radiologist
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpatients_radiology_reportssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field radiologist
        $this->radiologist->AdvancedSearch->SearchValue = @$filter["x_radiologist"];
        $this->radiologist->AdvancedSearch->SearchOperator = @$filter["z_radiologist"];
        $this->radiologist->AdvancedSearch->SearchCondition = @$filter["v_radiologist"];
        $this->radiologist->AdvancedSearch->SearchValue2 = @$filter["y_radiologist"];
        $this->radiologist->AdvancedSearch->SearchOperator2 = @$filter["w_radiologist"];
        $this->radiologist->AdvancedSearch->save();
    ...
    protected function setupListOptions()
    {
        ...
        $item->Visible = $Security->allowList(CurrentProjectID() . 'patients_radiology_reports_details');
        $item->OnLeft = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("patients_radiology_reports_details");
    ...
    public function renderListOptions()
    {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpatients_radiology_reportslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpatients_radiology_reportslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_patients_radiology_reports_details"
        $opt = $this->ListOptions["detail_patients_radiology_reports_details"];
        if ($Security->allowList(CurrentProjectID() . 'patients_radiology_reports_details')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("patients_radiology_reports_details", "TblCaption");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpatients_radiology_reportssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpatients_radiology_reportssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpatients_radiology_reportslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_patients_radiology_reports", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_patients_radiology_reports",
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->radiologist->setDbValue($row['radiologist']);
    ...
    protected function newRow()
    {
        ...
        $row['radiologist'] = $this->radiologist->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // radiologist
            $this->radiologist->ViewValue = $this->radiologist->CurrentValue;
            ...
            // radiologist
            $this->radiologist->HrefValue = "";
            $this->radiologist->TooltipValue = "";
