 ...
 * Page class
 */
class PatientsDischargeAdd extends PatientsDischarge
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientsDischargeAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "patientsdischargeadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("patientsdischargelist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "patientsdischargelist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "patientsdischargeview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "patientsdischargelist") {
                            ...
                            $returnUrl = "patientsdischargelist"; // Return list page content
                        }
                    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_ipd_vitals", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PatientIpdVitalsGrid");
            if (in_array("patient_ipd_vitals", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->admission_id->setSessionValue($this->admission_id->CurrentValue); // Set master key
                $detailPage->patient_id->setSessionValue($this->patient_id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "patient_ipd_vitals"); // Load user level of detail table
