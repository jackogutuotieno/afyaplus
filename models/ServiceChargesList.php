 ...
 * Page class
 */
class ServiceChargesList extends ServiceCharges
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServiceChargesList";
    ...
    public $FormName = "fservice_chargeslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "servicechargeslist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'service_charges';
        $this->TableName = 'service_charges';
        ...
        // Table object (service_charges)
        if (!isset($GLOBALS["service_charges"]) || $GLOBALS["service_charges"]::class == PROJECT_NAMESPACE . "service_charges") {
            $GLOBALS["service_charges"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "servicechargesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "servicechargesdelete";
        $this->MultiUpdateUrl = "servicechargesupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'service_charges');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "servicechargesview"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fservice_chargesgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fservice_chargessrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fservice_chargessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->service_category_id->AdvancedSearch->SearchValue = @$filter["x_service_category_id"];
        $this->service_category_id->AdvancedSearch->SearchOperator = @$filter["z_service_category_id"];
        $this->service_category_id->AdvancedSearch->SearchCondition = @$filter["v_service_category_id"];
        $this->service_category_id->AdvancedSearch->SearchValue2 = @$filter["y_service_category_id"];
        $this->service_category_id->AdvancedSearch->SearchOperator2 = @$filter["w_service_category_id"];
        $this->service_category_id->AdvancedSearch->save();

        // Field service_subcategory_id
        $this->service_subcategory_id->AdvancedSearch->SearchValue = @$filter["x_service_subcategory_id"];
        $this->service_subcategory_id->AdvancedSearch->SearchOperator = @$filter["z_service_subcategory_id"];
        $this->service_subcategory_id->AdvancedSearch->SearchCondition = @$filter["v_service_subcategory_id"];
        $this->service_subcategory_id->AdvancedSearch->SearchValue2 = @$filter["y_service_subcategory_id"];
        $this->service_subcategory_id->AdvancedSearch->SearchOperator2 = @$filter["w_service_subcategory_id"];
        $this->service_subcategory_id->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->service_category_id, $default, true); // service_category_id
        $this->buildSearchSql($where, $this->service_subcategory_id, $default, true); // service_subcategory_id
        $this->buildSearchSql($where, $this->service_name, $default, true); // service_name
        $this->buildSearchSql($where, $this->cost, $default, false); // cost
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->service_category_id->AdvancedSearch->save(); // service_category_id
            $this->service_subcategory_id->AdvancedSearch->save(); // service_subcategory_id
            $this->service_name->AdvancedSearch->save(); // service_name
    ...
    public function showFilterList()
    {
        ...
        // Field service_category_id
        $filter = $this->queryBuilderWhere("service_category_id");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->service_category_id, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->service_category_id->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field service_subcategory_id
        $filter = $this->queryBuilderWhere("service_subcategory_id");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->service_subcategory_id, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->service_subcategory_id->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field service_name
        $filter = $this->queryBuilderWhere("service_name");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->service_name, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->service_name->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field cost
        $filter = $this->queryBuilderWhere("cost");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->cost, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->cost->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->service_category_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->service_subcategory_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->service_name->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->service_category_id->AdvancedSearch->unsetSession();
        $this->service_subcategory_id->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->service_category_id->AdvancedSearch->load();
        $this->service_subcategory_id->AdvancedSearch->load();
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"service_charges\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"service_charges\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"service_charges\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fservice_chargeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fservice_chargeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"service_charges\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fservice_chargessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fservice_chargessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fservice_chargeslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_service_charges", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_service_charges",
    ...
    protected function loadSearchValues()
    {
        ...
        // service_category_id
        if ($this->service_category_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->service_category_id->AdvancedSearch->SearchValue != "" || $this->service_category_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // service_subcategory_id
        if ($this->service_subcategory_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->service_subcategory_id->AdvancedSearch->SearchValue != "" || $this->service_subcategory_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // service_name
        if ($this->service_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->service_name->AdvancedSearch->SearchValue != "" || $this->service_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cost
        if ($this->cost->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cost->AdvancedSearch->SearchValue != "" || $this->cost->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // service_subcategory_id
            $this->service_subcategory_id->HrefValue = "";
            $this->service_subcategory_id->TooltipValue = "";
            ...
            // service_category_id
            if ($this->service_category_id->UseFilter && !EmptyValue($this->service_category_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->service_category_id->AdvancedSearch->SearchValue)) {
                    $this->service_category_id->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->service_category_id->AdvancedSearch->SearchValue);
                }
                $this->service_category_id->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->service_category_id->AdvancedSearch->SearchValue);
            }

            // service_subcategory_id
            if ($this->service_subcategory_id->UseFilter && !EmptyValue($this->service_subcategory_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->service_subcategory_id->AdvancedSearch->SearchValue)) {
                    $this->service_subcategory_id->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->service_subcategory_id->AdvancedSearch->SearchValue);
                }
                $this->service_subcategory_id->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->service_subcategory_id->AdvancedSearch->SearchValue);
            }
            ...
            $this->cost->EditValue = $this->cost->AdvancedSearch->SearchValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fservice_chargeslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fservice_chargessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
