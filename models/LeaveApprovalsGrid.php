 ...
 * Page class
 */
class LeaveApprovalsGrid extends LeaveApprovals
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LeaveApprovalsGrid";
    ...
    public $FormName = "fleave_approvalsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "leaveapprovalsgrid";
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->leave_category_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->leave_category_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
                ...
                if ($curVal != "") {
                    ...
                    if ($this->user_id->EditValue === null) { // Lookup from database
                        ...
                        if ($ari > 0) { // Lookup values found
                            ...
                            $this->user_id->EditValue = HtmlEncode(FormatNumber($this->user_id->CurrentValue, $this->user_id->formatPattern()));
                        }
                    }
                } else {
            ...
            $curVal = strval($this->leave_category_id->CurrentValue);
            if ($curVal != "") {
                $this->leave_category_id->EditValue = $this->leave_category_id->lookupCacheOption($curVal);
                if ($this->leave_category_id->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->leave_category_id->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $curVal, $this->leave_category_id->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                    $sqlWrk = $this->leave_category_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->leave_category_id->Lookup->renderViewRow($rswrk[0]);
                        $this->leave_category_id->EditValue = $this->leave_category_id->displayValue($arwrk);
                    } else {
                        $this->leave_category_id->EditValue = HtmlEncode(FormatNumber($this->leave_category_id->CurrentValue, $this->leave_category_id->formatPattern()));
                    }
                }
            } else {
                $this->leave_category_id->EditValue = null;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->leave_category_id->FormValue)) {
                $this->leave_category_id->addErrorMessage($this->leave_category_id->getErrorMessage(false));
            }
