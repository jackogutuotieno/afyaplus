 ...
 * Page class
 */
class PatientsLabReportView extends PatientsLabReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientsLabReportView";
    ...
    // CSS class/style
    public $CurrentPageName = "patientslabreportview";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "patientslabreportlist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_urinalysis_results");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("urinalysis_results", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("urinalysisresultslist?" . Config("TABLE_SHOW_MASTER") . "=patients_lab_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("UrinalysisResultsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'patients_lab_report')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=urinalysis_results"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($item->Visible) {
            ...
            $detailTableLink .= "urinalysis_results";
        }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (urinalysis_results)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("urinalysis_results", explode(",", $this->getCurrentDetailTable()))) {
            $urinalysis_results = new UrinalysisResultsList();
            $rsdetail = $urinalysis_results->loadRs($urinalysis_results->getDetailFilterFromSession(), $urinalysis_results->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($urinalysis_results);
                    $urinalysis_results->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("urinalysis_results", $detailTblVar)) {
                $detailPageObj = Container("UrinalysisResultsGrid");
