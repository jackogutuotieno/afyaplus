 ...
 * Page class
 */
class ExpensesReportSummary extends ExpensesReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ExpensesReportSummary";
    ...
    public $CurrentPageName = "expensesreport";
    ...
    public function __construct()
    {
        ...
        // Table object (Expenses_Report)
        if (!isset($GLOBALS["Expenses_Report"]) || $GLOBALS["Expenses_Report"]::class == PROJECT_NAMESPACE . "Expenses_Report") {
            $GLOBALS["Expenses_Report"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Expenses Report');
        }
    ...
    public function loadRowValues($record)
    {
        $data = [];
        $data["id"] = $record['id'];
        $data["expense_title"] = $record['expense_title'];
        $data["description"] = $record['description'];
        $data["cost"] = $record['cost'];
        $data["date_created"] = $record['date_created'];
        $data["date_updated"] = $record['date_updated'];
        $data["expenses_month"] = $record['expenses_month'];
        $this->Rows[] = $data;
        $this->id->setDbValue($record['id']);
        $this->expense_title->setDbValue($record['expense_title']);
        ...
        $this->expenses_month->setDbValue($record['expenses_month']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            $this->expense_title->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            $this->expenses_month->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) { // Summary row
            ...
            // expense_title
            $currentValue = $this->expense_title->CurrentValue;
            $viewValue = &$this->expense_title->ViewValue;
            $viewAttrs = &$this->expense_title->ViewAttrs;
            $cellAttrs = &$this->expense_title->CellAttrs;
            $hrefValue = &$this->expense_title->HrefValue;
            $linkAttrs = &$this->expense_title->LinkAttrs;
            $this->cellRendered($this->expense_title, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // expenses_month
            $currentValue = $this->expenses_month->CurrentValue;
            $viewValue = &$this->expenses_month->ViewValue;
            $viewAttrs = &$this->expenses_month->ViewAttrs;
            $cellAttrs = &$this->expenses_month->CellAttrs;
            $hrefValue = &$this->expenses_month->HrefValue;
            $linkAttrs = &$this->expenses_month->LinkAttrs;
            $this->cellRendered($this->expenses_month, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        }
    ...
    protected function setupFieldCount()
    {
        ...
        if ($this->expenses_month->Visible) {
            $this->DetailColumnCount += 1;
        }
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fExpenses_Reportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fExpenses_Reportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
