 ...
 * Page class
 */
class PatientIpdPrescriptionsView extends PatientIpdPrescriptions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientIpdPrescriptionsView";
    ...
    // CSS class/style
    public $CurrentPageName = "patientipdprescriptionsview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "patientipdprescriptionslist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "patientipdprescriptionslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_patient_ipd_prescription_details");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("patient_ipd_prescription_details", "TblCaption");
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("patientipdprescriptiondetailslist?" . Config("TABLE_SHOW_MASTER") . "=patient_ipd_prescriptions&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("PatientIpdPrescriptionDetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'patient_ipd_prescriptions')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=patient_ipd_prescription_details"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "patient_ipd_prescription_details";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'patient_ipd_prescriptions')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=patient_ipd_prescription_details"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (patient_ipd_prescription_details)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("patient_ipd_prescription_details", explode(",", $this->getCurrentDetailTable()))) {
            $patient_ipd_prescription_details = new PatientIpdPrescriptionDetailsList();
            $rsdetail = $patient_ipd_prescription_details->loadRs($patient_ipd_prescription_details->getDetailFilterFromSession(), $patient_ipd_prescription_details->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($patient_ipd_prescription_details);
                    $patient_ipd_prescription_details->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("patient_ipd_prescription_details", $detailTblVar)) {
                $detailPageObj = Container("PatientIpdPrescriptionDetailsGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->prescription_id->IsDetailKey = true;
                    $detailPageObj->prescription_id->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->prescription_id->setSessionValue($detailPageObj->prescription_id->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("patientipdprescriptionslist"), "", $this->TableVar, true);
        $pageId = "view";
