...
class IpdBillingReport extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->patient_uhid->IsPrimaryKey = true; // Primary key field
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('patient_uhid', $rs)) {
                AddFilter($where, QuotedName('patient_uhid', $this->Dbid) . '=' . QuotedValue($rs['patient_uhid'], $this->patient_uhid->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "`patient_uhid` = @patient_uhid@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->patient_uhid->CurrentValue : $this->patient_uhid->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->patient_uhid->CurrentValue = $keys[0];
            } else {
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('patient_uhid', $row) ? $row['patient_uhid'] : null;
        } else {
            $val = !EmptyValue($this->patient_uhid->OldValue) && !$current ? $this->patient_uhid->OldValue : $this->patient_uhid->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@patient_uhid@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "\"patient_uhid\":" . VarToJson($this->patient_uhid->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->patient_uhid->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->patient_uhid->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("patient_uhid") ?? Route("patient_uhid")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->patient_uhid->CurrentValue = $key;
            } else {
                $this->patient_uhid->OldValue = $key;
            }
