...
class LaboratoryBillingReportGrid extends LaboratoryBillingReport
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'id1' first before field var 'x_id1'
        $val = $CurrentForm->hasValue("id1") ? $CurrentForm->getValue("id1") : $CurrentForm->getValue("x_id1");
        if (!$this->id1->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->id1->setFormValue($val);
        }
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->id1->Visible && $this->id1->Required) {
                if (!$this->id1->IsDetailKey && EmptyValue($this->id1->FormValue)) {
                    $this->id1->addErrorMessage(str_replace("%s", $this->id1->caption(), $this->id1->RequiredErrorMessage));
                }
            }
