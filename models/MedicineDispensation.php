...
/**
 * Table class for medicine_dispensation
 */
class MedicineDispensation extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "patient_visits") {
            ...
            if ($this->visit_id->getSessionValue() != "") {
                $masterFilter .= " AND " . GetKeyFilter($masterTable->id, $this->visit_id->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "patient_visits") {
            ...
            if ($this->visit_id->getSessionValue() != "") {
                $detailFilter .= " AND " . GetKeyFilter($this->visit_id, $this->visit_id->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "patient_visits":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->patient_id, $keys["patient_id"], $this->patient_id->DataType, $this->Dbid) . " AND " . GetKeyFilter($masterTable->id, $keys["visit_id"], $this->visit_id->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "patient_visits":
                return GetKeyFilter($this->patient_id, $masterTable->patient_id->DbValue, $masterTable->patient_id->DataType, $masterTable->Dbid) . " AND " . GetKeyFilter($this->visit_id, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "medicine_dispensation";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("medicinedispensationlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "medicinedispensationview" => $Language->phrase("View"),
            "medicinedispensationedit" => $Language->phrase("Edit"),
            "medicinedispensationadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "medicinedispensationlist";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "MedicineDispensationView",
            Config("API_ADD_ACTION") => "MedicineDispensationAdd",
            Config("API_EDIT_ACTION") => "MedicineDispensationEdit",
            Config("API_DELETE_ACTION") => "MedicineDispensationDelete",
            Config("API_LIST_ACTION") => "MedicineDispensationList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("medicinedispensationview", $parm);
        } else {
            $url = $this->keyUrl("medicinedispensationview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "medicinedispensationadd?" . $parm;
        } else {
            $url = "medicinedispensationadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("medicinedispensationlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("medicinedispensationdelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "MedicineDispensationList";
    ...
    {
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM medicine_dispensation";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'medicine_dispensation');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'medicine_dispensation', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'medicine_dispensation', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'medicine_dispensation', $fldname, $key, $oldvalue);
            }
        }
    }
