 ...
 * Page class
 */
class OpdBillMasterReportList extends OpdBillMasterReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "OpdBillMasterReportList";
    ...
    public $FormName = "fopd_bill_master_reportlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "opdbillmasterreportlist";
    ...
    public function setVisibility()
    {
        ...
        $this->uhid->Visible = false;
        $this->patient_name->setVisibility();
        $this->age->Visible = false;
        $this->gender->Visible = false;
        $this->visit_type->Visible = false;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'opd_bill_master_report';
        $this->TableName = 'opd_bill_master_report';
        ...
        // Table object (opd_bill_master_report)
        if (!isset($GLOBALS["opd_bill_master_report"]) || $GLOBALS["opd_bill_master_report"]::class == PROJECT_NAMESPACE . "opd_bill_master_report") {
            $GLOBALS["opd_bill_master_report"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "opdbillmasterreportadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "opdbillmasterreportdelete";
        $this->MultiUpdateUrl = "opdbillmasterreportupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'opd_bill_master_report');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "opdbillmasterreportview"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fopd_bill_master_reportgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fopd_bill_master_reportsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->uhid->AdvancedSearch->toJson(), ","); // Field uhid
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fopd_bill_master_reportsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field uhid
        $this->uhid->AdvancedSearch->SearchValue = @$filter["x_uhid"];
        $this->uhid->AdvancedSearch->SearchOperator = @$filter["z_uhid"];
        $this->uhid->AdvancedSearch->SearchCondition = @$filter["v_uhid"];
        $this->uhid->AdvancedSearch->SearchValue2 = @$filter["y_uhid"];
        $this->uhid->AdvancedSearch->SearchOperator2 = @$filter["w_uhid"];
        $this->uhid->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->uhid, $default, false); // uhid
        ...
        $this->buildSearchSql($where, $this->visit_type, $default, false); // visit_type
        $this->buildSearchSql($where, $this->payment_method, $default, true); // payment_method
        $this->buildSearchSql($where, $this->company, $default, true); // company
    ...
    public function showFilterList()
    {
        ...
        // Field payment_method
        $filter = $this->queryBuilderWhere("payment_method");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->payment_method, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->payment_method->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field company
        $filter = $this->queryBuilderWhere("company");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->company, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->company->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field visit_date
        $filter = $this->queryBuilderWhere("visit_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->visit_date, false, false);
        }
        if ($filter != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->uhid->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->visit_type->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->uhid->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->visit_date->Expression . " DESC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_opd_consultation"
        $item = &$this->ListOptions->add("detail_opd_consultation");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'opd_consultation');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_opd_lab_master_bill"
        $item = &$this->ListOptions->add("detail_opd_lab_master_bill");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'opd_lab_master_bill');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"opd_bill_master_report\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fopd_bill_master_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fopd_bill_master_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_opd_consultation"
        $opt = $this->ListOptions["detail_opd_consultation"];
        if ($Security->allowList(CurrentProjectID() . 'opd_consultation')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("opd_consultation", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("opdconsultationlist?" . Config("TABLE_SHOW_MASTER") . "=opd_bill_master_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("OpdConsultationGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'opd_bill_master_report')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=opd_consultation");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "opd_consultation";
            }
            if ($links != "") {
        ...
        // "detail_opd_lab_master_bill"
        $opt = $this->ListOptions["detail_opd_lab_master_bill"];
        if ($Security->allowList(CurrentProjectID() . 'opd_lab_master_bill')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("opd_lab_master_bill", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("opdlabmasterbilllist?" . Config("TABLE_SHOW_MASTER") . "=opd_bill_master_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("OpdLabMasterBillGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'opd_bill_master_report')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=opd_lab_master_bill");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "opd_lab_master_bill";
            }
            if ($links != "") {
                $body .= "<button type=\"button\" class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        if ($Security->allowList(CurrentProjectID() . 'opd_pharmacy_master_bill')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("opd_pharmacy_master_bill", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("opdpharmacymasterbilllist?" . Config("TABLE_SHOW_MASTER") . "=opd_bill_master_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("OpdPharmacyMasterBillGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'opd_bill_master_report')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=opd_pharmacy_master_bill");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "payment_method");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fopd_bill_master_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fopd_bill_master_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fopd_bill_master_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_opd_bill_master_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_opd_bill_master_report",
    ...
    protected function loadSearchValues()
    {
        ...
        // uhid
        if ($this->uhid->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->uhid->AdvancedSearch->SearchValue != "" || $this->uhid->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // visit_type
        if ($this->visit_type->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->visit_type->AdvancedSearch->SearchValue != "" || $this->visit_type->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['uhid'] = $this->uhid->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // uhid
            $this->uhid->ViewValue = $this->uhid->CurrentValue;
            ...
            $this->visit_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->visit_date->AdvancedSearch->SearchValue, $this->visit_date->formatPattern()), $this->visit_date->formatPattern()));
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fopd_bill_master_reportlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fopd_bill_master_reportlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fopd_bill_master_reportlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fopd_bill_master_reportlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fopd_bill_master_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
