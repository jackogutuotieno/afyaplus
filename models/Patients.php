...
class Patients extends DbTable
{
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "patients_dependants") {
            $detailUrl = Container("patients_dependants")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'patients');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'patients', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'patients', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'patients', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserted($rsold, $rsnew)
    {
        ...
        $this->setSuccessMessage("Patient successfully registered.");
    }
    ...
    {
        $this->id->ViewValue = '<a href="patientsview/' . $this->id->ViewValue . '?showdetail=patient_appointments,patient_visits,patients_dependants" target="_blank"> ' . $this->id->ViewValue . ' </a>';
    }
