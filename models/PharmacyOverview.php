 ...
 * Page class
 */
class PharmacyOverview extends ReportTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PharmacyOverview";
    ...
    public $CurrentPageName = "pharmacyoverview";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Pharmacy_Overview';
        $this->TableName = 'Pharmacy Overview';
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Pharmacy Overview');
        }
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("MedicineStockReportSummary");
                $Page->run();
                $Page->StockUpdatebyMonth->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->StockUpdatebyMonth->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("MedicineStockReportSummary");
                $Page->run();
                $Page->StockbySupplier->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->StockbySupplier->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 3) {
                $Page = Container("MedicineStockReportSummary");
                $Page->run();
                $Page->StockbyExpiryStatus->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->StockbyExpiryStatus->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 4) {
                $Page = Container("MedicineStockReportSummary");
                $Page->run();
                $Page->StockbyMedicineBrand->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->StockbyMedicineBrand->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
