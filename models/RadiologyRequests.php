...
/**
 * Table class for radiology_requests
 */
class RadiologyRequests extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "radiology_requests_details") {
            $detailUrl = Container("radiology_requests_details")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "radiologyrequestslist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "radiology_requests";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("radiologyrequestslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "radiologyrequestsview" => $Language->phrase("View"),
            "radiologyrequestsedit" => $Language->phrase("Edit"),
            "radiologyrequestsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "RadiologyRequestsView",
            Config("API_ADD_ACTION") => "RadiologyRequestsAdd",
            Config("API_EDIT_ACTION") => "RadiologyRequestsEdit",
            Config("API_DELETE_ACTION") => "RadiologyRequestsDelete",
            Config("API_LIST_ACTION") => "RadiologyRequestsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("radiologyrequestsview", $parm);
        } else {
            $url = $this->keyUrl("radiologyrequestsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "radiologyrequestsadd?" . $parm;
        } else {
            $url = "radiologyrequestsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("radiologyrequestsedit", $parm);
        } else {
            $url = $this->keyUrl("radiologyrequestsedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyrequestslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("radiologyrequestsadd", $parm);
        } else {
            $url = $this->keyUrl("radiologyrequestsadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyrequestslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("radiologyrequestsdelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "RadiologyRequestsList";
    ...
    {
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM radiology_requests";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'radiology_requests');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'radiology_requests', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'radiology_requests', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'radiology_requests', $fldname, $key, $oldvalue);
            }
        }
    }
