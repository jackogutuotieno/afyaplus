 ...
 * Page class
 */
class ServiceChargesEdit extends ServiceCharges
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServiceChargesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "servicechargesedit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("servicechargeslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "servicechargeslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "servicechargeslist") {
                            ...
                            $returnUrl = "servicechargeslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'service_category_id' first before field var 'x_service_category_id'
        $val = $CurrentForm->hasValue("service_category_id") ? $CurrentForm->getValue("service_category_id") : $CurrentForm->getValue("x_service_category_id");
        if (!$this->service_category_id->IsDetailKey) {
            if (IsApi() && $val === null) {
        ...
        // Check field name 'service_subcategory_id' first before field var 'x_service_subcategory_id'
        $val = $CurrentForm->hasValue("service_subcategory_id") ? $CurrentForm->getValue("service_subcategory_id") : $CurrentForm->getValue("x_service_subcategory_id");
        if (!$this->service_subcategory_id->IsDetailKey) {
            if (IsApi() && $val === null) {
    ...
    public function renderRow()
    {
        ...
        // service_category_id
        $this->service_category_id->RowCssClass = "row";

        // service_subcategory_id
        $this->service_subcategory_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->service_category_id->Visible && $this->service_category_id->Required) {
                if (!$this->service_category_id->IsDetailKey && EmptyValue($this->service_category_id->FormValue)) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // service_category_id
        $this->service_category_id->setDbValueDef($rsnew, $this->service_category_id->CurrentValue, $this->service_category_id->ReadOnly);

        // service_subcategory_id
        $this->service_subcategory_id->setDbValueDef($rsnew, $this->service_subcategory_id->CurrentValue, $this->service_subcategory_id->ReadOnly);
    ...
    {
        if (isset($row['service_category_id'])) { // service_category_id
            $this->service_category_id->CurrentValue = $row['service_category_id'];
        }
        if (isset($row['service_subcategory_id'])) { // service_subcategory_id
            $this->service_subcategory_id->CurrentValue = $row['service_subcategory_id'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("servicechargeslist"), "", $this->TableVar, true);
        $pageId = "edit";
