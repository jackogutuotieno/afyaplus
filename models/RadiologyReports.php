...
/**
 * Table class for radiology_reports
 */
class RadiologyReports extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "radiology_reports";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "radiology_reports";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("radiologyreportslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "radiologyreportsview" => $Language->phrase("View"),
            "radiologyreportsedit" => $Language->phrase("Edit"),
            "radiologyreportsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "RadiologyReportsView",
            Config("API_ADD_ACTION") => "RadiologyReportsAdd",
            Config("API_EDIT_ACTION") => "RadiologyReportsEdit",
            Config("API_DELETE_ACTION") => "RadiologyReportsDelete",
            Config("API_LIST_ACTION") => "RadiologyReportsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("radiologyreportsview", $parm);
        } else {
            $url = $this->keyUrl("radiologyreportsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "radiologyreportsadd?" . $parm;
        } else {
            $url = "radiologyreportsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyreportsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyreportslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyreportsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("radiologyreportslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("radiologyreportsdelete", $parm);
        }
    }
    ...
    {
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM radiology_reports";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'radiology_reports');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'radiology_reports', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'radiology_reports', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'radiology_reports', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $if_exists = ExecuteScalar("SELECT count(*) FROM radiology_reports where radiology_requests_id = '".$rsnew["radiology_requests_id"]."'");
    	if($if_exists > 0) { // Check for double applications
    		$this->CancelMessage = "Report has already been submitted for this request";
    		return false;
    	} else if ($if_exists < 0) { 
            return true; 
        }
