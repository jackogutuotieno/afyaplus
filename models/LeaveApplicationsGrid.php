 ...
 * Page class
 */
class LeaveApplicationsGrid extends LeaveApplications
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LeaveApplicationsGrid";
    ...
    public $FormName = "fleave_applicationsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "leaveapplicationsgrid";
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_leave_category_id") &&
            $CurrentForm->hasValue("o_leave_category_id") &&
            $this->leave_category_id->CurrentValue != $this->leave_category_id->DefaultValue &&
            !($this->leave_category_id->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->leave_category_id->CurrentValue == $this->leave_category_id->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_start_from_date") &&
            $CurrentForm->hasValue("o_start_from_date") &&
            $this->start_from_date->CurrentValue != $this->start_from_date->DefaultValue &&
            !($this->start_from_date->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->start_from_date->CurrentValue == $this->start_from_date->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_days_applied") &&
            $CurrentForm->hasValue("o_days_applied") &&
            $this->days_applied->CurrentValue != $this->days_applied->DefaultValue &&
            !($this->days_applied->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->days_applied->CurrentValue == $this->days_applied->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_reporting_date") &&
            $CurrentForm->hasValue("o_reporting_date") &&
            $this->reporting_date->CurrentValue != $this->reporting_date->DefaultValue &&
            !($this->reporting_date->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->reporting_date->CurrentValue == $this->reporting_date->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'leave_category_id' first before field var 'x_leave_category_id'
        $val = $CurrentForm->hasValue("leave_category_id") ? $CurrentForm->getValue("leave_category_id") : $CurrentForm->getValue("x_leave_category_id");
        if (!$this->leave_category_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->leave_category_id->Visible = false; // Disable update for API request
            } else {
                $this->leave_category_id->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_leave_category_id")) {
            $this->leave_category_id->setOldValue($CurrentForm->getValue("o_leave_category_id"));
        }
        ...
        if ($CurrentForm->hasValue("o_start_from_date")) {
            $this->start_from_date->setOldValue($CurrentForm->getValue("o_start_from_date"));
        }
        ...
        if (!$this->days_applied->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->days_applied->Visible = false; // Disable update for API request
            } else {
                $this->days_applied->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_days_applied")) {
            $this->days_applied->setOldValue($CurrentForm->getValue("o_days_applied"));
        }

        // Check field name 'reporting_date' first before field var 'x_reporting_date'
        $val = $CurrentForm->hasValue("reporting_date") ? $CurrentForm->getValue("reporting_date") : $CurrentForm->getValue("x_reporting_date");
        if (!$this->reporting_date->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->reporting_date->Visible = false; // Disable update for API request
            } else {
                $this->reporting_date->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_reporting_date")) {
            $this->reporting_date->setOldValue($CurrentForm->getValue("o_reporting_date"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->reporting_date->CurrentValue = $this->reporting_date->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->leave_category_id->CurrentValue));
            if ($curVal != "") {
                $this->leave_category_id->ViewValue = $this->leave_category_id->lookupCacheOption($curVal);
            } else {
                $this->leave_category_id->ViewValue = $this->leave_category_id->Lookup !== null && is_array($this->leave_category_id->lookupOptions()) && count($this->leave_category_id->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->leave_category_id->ViewValue !== null) { // Load from cache
                $this->leave_category_id->EditValue = array_values($this->leave_category_id->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->leave_category_id->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->leave_category_id->CurrentValue, $this->leave_category_id->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
                $sqlWrk = $this->leave_category_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->leave_category_id->EditValue = $arwrk;
            }
            ...
            $this->reporting_date->EditValue = HtmlEncode($this->reporting_date->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->leave_category_id->Visible && $this->leave_category_id->Required) {
            ...
            if ($this->days_applied->Visible && $this->days_applied->Required) {
                if (!$this->days_applied->IsDetailKey && EmptyValue($this->days_applied->FormValue)) {
                    $this->days_applied->addErrorMessage(str_replace("%s", $this->days_applied->caption(), $this->days_applied->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->days_applied->FormValue)) {
                $this->days_applied->addErrorMessage($this->days_applied->getErrorMessage(false));
            }
            if ($this->reporting_date->Visible && $this->reporting_date->Required) {
                if (!$this->reporting_date->IsDetailKey && EmptyValue($this->reporting_date->FormValue)) {
                    $this->reporting_date->addErrorMessage(str_replace("%s", $this->reporting_date->caption(), $this->reporting_date->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // reporting_date
        $this->reporting_date->setDbValueDef($rsnew, $this->reporting_date->CurrentValue, $this->reporting_date->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['reporting_date'])) { // reporting_date
            $this->reporting_date->CurrentValue = $row['reporting_date'];
        }
    ...
    protected function getAddRow()
    {
        ...
        // leave_category_id
        $this->leave_category_id->setDbValueDef($rsnew, $this->leave_category_id->CurrentValue, false);

        // start_from_date
        $this->start_from_date->setDbValueDef($rsnew, UnFormatDateTime($this->start_from_date->CurrentValue, $this->start_from_date->formatPattern()), false);

        // days_applied
        $this->days_applied->setDbValueDef($rsnew, $this->days_applied->CurrentValue, false);

        // reporting_date
        $this->reporting_date->setDbValueDef($rsnew, $this->reporting_date->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['leave_category_id'])) { // leave_category_id
            $this->leave_category_id->setFormValue($row['leave_category_id']);
        }
        if (isset($row['start_from_date'])) { // start_from_date
            $this->start_from_date->setFormValue($row['start_from_date']);
        }
        if (isset($row['days_applied'])) { // days_applied
            $this->days_applied->setFormValue($row['days_applied']);
        }
        if (isset($row['reporting_date'])) { // reporting_date
            $this->reporting_date->setFormValue($row['reporting_date']);
        }
        if (isset($row['status'])) { // status
