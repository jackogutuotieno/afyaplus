...
/**
 * Table class for patients_discharge
 */
class PatientsDischarge extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "patients_discharge";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // discharge
        $this->discharge = new DbField(
            $this, // Table
            'x_discharge', // Variable name
            'discharge', // Name
            '`discharge`', // Expression
            '`discharge`', // Basic search expression
            16, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`discharge`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->discharge->InputTextType = "text";
        $this->discharge->Raw = true;
        $this->discharge->Nullable = false; // NOT NULL field
        $this->discharge->Required = true; // Required field
        $this->discharge->setDataType(DataType::BOOLEAN);
        $this->discharge->Lookup = new Lookup($this->discharge, 'patients_discharge', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->discharge->OptionCount = 2;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "patients_discharge";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("patientsdischargelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "patientsdischargeview" => $Language->phrase("View"),
            "patientsdischargeedit" => $Language->phrase("Edit"),
            "patientsdischargeadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PatientsDischargeView",
            Config("API_ADD_ACTION") => "PatientsDischargeAdd",
            Config("API_EDIT_ACTION") => "PatientsDischargeEdit",
            Config("API_DELETE_ACTION") => "PatientsDischargeDelete",
            Config("API_LIST_ACTION") => "PatientsDischargeList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("patientsdischargeview", $parm);
        } else {
            $url = $this->keyUrl("patientsdischargeview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "patientsdischargeadd?" . $parm;
        } else {
            $url = "patientsdischargeadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdischargelist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdischargelist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("patientsdischargedelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "PatientsDischargeList";
