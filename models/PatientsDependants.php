...
/**
 * Table class for patients_dependants
 */
class PatientsDependants extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "patients_dependants";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // relationship
        $this->relationship = new DbField(
            $this, // Table
            'x_relationship', // Variable name
            'relationship', // Name
            '`relationship`', // Expression
            '`relationship`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`relationship`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->relationship->InputTextType = "text";
        $this->relationship->Nullable = false; // NOT NULL field
        $this->relationship->Required = true; // Required field
        $this->relationship->setSelectMultiple(false); // Select one
        $this->relationship->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->relationship->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->relationship->Lookup = new Lookup($this->relationship, 'patients_dependants', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->relationship->OptionCount = 4;
        $this->relationship->SearchOperators = ["=", "<>"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "patients_dependants";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("patientsdependantslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "patientsdependantsview" => $Language->phrase("View"),
            "patientsdependantsedit" => $Language->phrase("Edit"),
            "patientsdependantsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PatientsDependantsView",
            Config("API_ADD_ACTION") => "PatientsDependantsAdd",
            Config("API_EDIT_ACTION") => "PatientsDependantsEdit",
            Config("API_DELETE_ACTION") => "PatientsDependantsDelete",
            Config("API_LIST_ACTION") => "PatientsDependantsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("patientsdependantsview", $parm);
        } else {
            $url = $this->keyUrl("patientsdependantsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "patientsdependantsadd?" . $parm;
        } else {
            $url = "patientsdependantsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdependantsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdependantslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdependantsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientsdependantslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("patientsdependantsdelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "PatientsDependantsList";
