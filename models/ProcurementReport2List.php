 ...
 * Page class
 */
class ProcurementReport2List extends ProcurementReport2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProcurementReport2List";
    ...
    public $FormName = "fprocurement_report2list";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "procurementreport2list";
    ...
    public function setVisibility()
    {
        ...
        $this->purchase_month->Visible = false;
        $this->purchase_year->Visible = false;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'procurement_report2';
        $this->TableName = 'procurement_report';
        ...
        // Table object (procurement_report2)
        if (!isset($GLOBALS["procurement_report2"]) || $GLOBALS["procurement_report2"]::class == PROJECT_NAMESPACE . "procurement_report2") {
            $GLOBALS["procurement_report2"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "procurementreport2add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "procurementreport2delete";
        $this->MultiUpdateUrl = "procurementreport2update";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'procurement_report');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "procurementreport2view"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fprocurement_report2grid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fprocurement_report2srch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->batch_number->AdvancedSearch->toJson(), ","); // Field batch_number
        $filterList = Concat($filterList, $this->supplier_name->AdvancedSearch->toJson(), ","); // Field supplier_name
        ...
        $filterList = Concat($filterList, $this->purchase_month->AdvancedSearch->toJson(), ","); // Field purchase_month
        $filterList = Concat($filterList, $this->purchase_year->AdvancedSearch->toJson(), ","); // Field purchase_year
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fprocurement_report2srch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->purchase_month->AdvancedSearch->SearchValue = @$filter["x_purchase_month"];
        $this->purchase_month->AdvancedSearch->SearchOperator = @$filter["z_purchase_month"];
        $this->purchase_month->AdvancedSearch->SearchCondition = @$filter["v_purchase_month"];
        $this->purchase_month->AdvancedSearch->SearchValue2 = @$filter["y_purchase_month"];
        $this->purchase_month->AdvancedSearch->SearchOperator2 = @$filter["w_purchase_month"];
        $this->purchase_month->AdvancedSearch->save();

        // Field purchase_year
        $this->purchase_year->AdvancedSearch->SearchValue = @$filter["x_purchase_year"];
        $this->purchase_year->AdvancedSearch->SearchOperator = @$filter["z_purchase_year"];
        $this->purchase_year->AdvancedSearch->SearchCondition = @$filter["v_purchase_year"];
        $this->purchase_year->AdvancedSearch->SearchValue2 = @$filter["y_purchase_year"];
        $this->purchase_year->AdvancedSearch->SearchOperator2 = @$filter["w_purchase_year"];
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->batch_number, $default, true); // batch_number
        $this->buildSearchSql($where, $this->supplier_name, $default, true); // supplier_name
        $this->buildSearchSql($where, $this->category_name, $default, true); // category_name
        $this->buildSearchSql($where, $this->subcategory_name, $default, true); // subcategory_name
        $this->buildSearchSql($where, $this->item_title, $default, false); // item_title
        $this->buildSearchSql($where, $this->quantity, $default, false); // quantity
        $this->buildSearchSql($where, $this->measuring_unit, $default, false); // measuring_unit
        $this->buildSearchSql($where, $this->unit_price, $default, false); // unit_price
        $this->buildSearchSql($where, $this->selling_price, $default, false); // selling_price
        $this->buildSearchSql($where, $this->amount_paid, $default, false); // amount_paid
        $this->buildSearchSql($where, $this->purchase_month, $default, true); // purchase_month
        $this->buildSearchSql($where, $this->purchase_year, $default, true); // purchase_year
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->batch_number->AdvancedSearch->save(); // batch_number
            $this->supplier_name->AdvancedSearch->save(); // supplier_name
            $this->category_name->AdvancedSearch->save(); // category_name
            $this->subcategory_name->AdvancedSearch->save(); // subcategory_name
            $this->item_title->AdvancedSearch->save(); // item_title
            $this->quantity->AdvancedSearch->save(); // quantity
            $this->measuring_unit->AdvancedSearch->save(); // measuring_unit
            ...
            $this->purchase_month->AdvancedSearch->save(); // purchase_month
            $this->purchase_year->AdvancedSearch->save(); // purchase_year
    ...
    public function showFilterList()
    {
        ...
        // Field batch_number
        $filter = $this->queryBuilderWhere("batch_number");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->batch_number, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->batch_number->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field supplier_name
        $filter = $this->queryBuilderWhere("supplier_name");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->supplier_name, false, true);
        }
        if ($filter != "") {
        ...
        // Field category_name
        $filter = $this->queryBuilderWhere("category_name");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->category_name, false, true);
        }
        if ($filter != "") {
        ...
        // Field subcategory_name
        $filter = $this->queryBuilderWhere("subcategory_name");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->subcategory_name, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->subcategory_name->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field item_title
        $filter = $this->queryBuilderWhere("item_title");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->item_title, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->item_title->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field measuring_unit
        $filter = $this->queryBuilderWhere("measuring_unit");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->measuring_unit, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->measuring_unit->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field unit_price
        $filter = $this->queryBuilderWhere("unit_price");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->unit_price, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->unit_price->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field selling_price
        $filter = $this->queryBuilderWhere("selling_price");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->selling_price, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->selling_price->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field amount_paid
        $filter = $this->queryBuilderWhere("amount_paid");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->amount_paid, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->amount_paid->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->batch_number->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->supplier_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->category_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->subcategory_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->item_title->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->quantity->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->measuring_unit->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->purchase_month->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->purchase_year->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->item_title->AdvancedSearch->unsetSession();
        $this->quantity->AdvancedSearch->unsetSession();
        $this->measuring_unit->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->item_title->AdvancedSearch->load();
        $this->quantity->AdvancedSearch->load();
        $this->measuring_unit->AdvancedSearch->load();
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprocurement_report2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprocurement_report2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fprocurement_report2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fprocurement_report2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fprocurement_report2list"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_procurement_report2", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_procurement_report2",
    ...
    protected function loadSearchValues()
    {
        ...
        // batch_number
        if ($this->batch_number->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->batch_number->AdvancedSearch->SearchValue != "" || $this->batch_number->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // supplier_name
        if ($this->supplier_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->supplier_name->AdvancedSearch->SearchValue != "" || $this->supplier_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // category_name
        if ($this->category_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->category_name->AdvancedSearch->SearchValue != "" || $this->category_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // subcategory_name
        if ($this->subcategory_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->subcategory_name->AdvancedSearch->SearchValue != "" || $this->subcategory_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // item_title
        if ($this->item_title->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->item_title->AdvancedSearch->SearchValue != "" || $this->item_title->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // measuring_unit
        if ($this->measuring_unit->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->measuring_unit->AdvancedSearch->SearchValue != "" || $this->measuring_unit->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // unit_price
        if ($this->unit_price->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->unit_price->AdvancedSearch->SearchValue != "" || $this->unit_price->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // selling_price
        if ($this->selling_price->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->selling_price->AdvancedSearch->SearchValue != "" || $this->selling_price->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // amount_paid
        if ($this->amount_paid->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // purchase_month
        if ($this->purchase_month->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->purchase_month->AdvancedSearch->SearchValue != "" || $this->purchase_month->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // purchase_year
        if ($this->purchase_year->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->purchase_year->AdvancedSearch->SearchValue != "" || $this->purchase_year->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->purchase_month->setDbValue($row['purchase_month']);
        $this->purchase_year->setDbValue($row['purchase_year']);
    ...
    protected function newRow()
    {
        ...
        $row['supplier_name'] = $this->supplier_name->DefaultValue;
        ...
        $row['purchase_month'] = $this->purchase_month->DefaultValue;
        $row['purchase_year'] = $this->purchase_year->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->item_title->Raw) {
                $this->item_title->AdvancedSearch->SearchValue = HtmlDecode($this->item_title->AdvancedSearch->SearchValue);
            }
            $this->item_title->EditValue = HtmlEncode($this->item_title->AdvancedSearch->SearchValue);
            ...
            if (!$this->measuring_unit->Raw) {
                $this->measuring_unit->AdvancedSearch->SearchValue = HtmlDecode($this->measuring_unit->AdvancedSearch->SearchValue);
            }
            $this->measuring_unit->EditValue = HtmlEncode($this->measuring_unit->AdvancedSearch->SearchValue);
            $this->measuring_unit->PlaceHolder = RemoveHtml($this->measuring_unit->caption());

            // unit_price
            $this->unit_price->setupEditAttributes();
            $this->unit_price->EditValue = $this->unit_price->AdvancedSearch->SearchValue;
            $this->unit_price->PlaceHolder = RemoveHtml($this->unit_price->caption());

            // selling_price
            $this->selling_price->setupEditAttributes();
            $this->selling_price->EditValue = $this->selling_price->AdvancedSearch->SearchValue;
            $this->selling_price->PlaceHolder = RemoveHtml($this->selling_price->caption());

            // amount_paid
            $this->amount_paid->setupEditAttributes();
            $this->amount_paid->EditValue = $this->amount_paid->AdvancedSearch->SearchValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fprocurement_report2list" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprocurement_report2srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
