...
class Beds extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // bed_charges
        $this->bed_charges = new DbField(
            $this, // Table
            'x_bed_charges', // Variable name
            'bed_charges', // Name
            '`bed_charges`', // Expression
            '`bed_charges`', // Basic search expression
            5, // Type
            22, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`bed_charges`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->bed_charges->InputTextType = "text";
        $this->bed_charges->Raw = true;
        $this->bed_charges->Nullable = false; // NOT NULL field
        $this->bed_charges->Required = true; // Required field
        $this->bed_charges->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->bed_charges->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->bed_charges->CustomMsg = $Language->fieldPhrase($this->TableVar, $this->bed_charges->Param, "CustomMsg");
        $this->Fields['bed_charges'] = &$this->bed_charges;
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "bedsview" => $Language->phrase("View"),
            "bedsedit" => $Language->phrase("Edit"),
            "bedsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "BedsView",
            Config("API_ADD_ACTION") => "BedsAdd",
            Config("API_EDIT_ACTION") => "BedsEdit",
            Config("API_DELETE_ACTION") => "BedsDelete",
            Config("API_LIST_ACTION") => "BedsList",
            default => ""
        };
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("bedsdelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'beds');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'beds', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'beds', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'beds', $fldname, $key, $oldvalue);
            }
        }
    }
