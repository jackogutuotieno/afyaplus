 ...
 * Page class
 */
class InvoiceReportView extends InvoiceReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvoiceReportView";
    ...
    // CSS class/style
    public $CurrentPageName = "invoicereportview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "invoicereportlist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "invoicereportlist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_invoice_report_details");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("invoice_report_details", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("invoicereportdetailslist?" . Config("TABLE_SHOW_MASTER") . "=invoice_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("InvoiceReportDetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'invoice_report')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=invoice_report_details"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($item->Visible) {
            ...
            $detailTableLink .= "invoice_report_details";
        }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (invoice_report_details)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("invoice_report_details", explode(",", $this->getCurrentDetailTable()))) {
            $invoice_report_details = new InvoiceReportDetailsList();
            $rsdetail = $invoice_report_details->loadRs($invoice_report_details->getDetailFilterFromSession(), $invoice_report_details->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($invoice_report_details);
                    $invoice_report_details->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("invoice_report_details", $detailTblVar)) {
                $detailPageObj = Container("InvoiceReportDetailsGrid");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("invoicereportlist"), "", $this->TableVar, true);
        $pageId = "view";
