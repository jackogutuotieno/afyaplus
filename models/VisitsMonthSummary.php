 ...
 * Page class
 */
class VisitsMonthSummary extends VisitsMonth
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VisitsMonthSummary";
    ...
    public $CurrentPageName = "visitsmonth";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Visits_Month';
        $this->TableName = 'Visits Month';
        ...
        // Table object (Visits_Month)
        if (!isset($GLOBALS["Visits_Month"]) || $GLOBALS["Visits_Month"]::class == PROJECT_NAMESPACE . "Visits_Month") {
            $GLOBALS["Visits_Month"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Visits Month');
        }
    ...
    public function loadRowValues($record)
    {
        ...
        $this->payment_method->setDbValue($record['payment_method']);
        $this->company->setDbValue($record['company']);
        $this->date_created->setDbValue($record['date_created']);
        $this->date_updated->setDbValue($record['date_updated']);
        $this->visit_month->setDbValue($record['visit_month']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            $this->patient_name_visits->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            $this->payment_method->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");

            // company
            $this->company->ViewValue = $this->company->CurrentValue;
            $this->company->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) { // Summary row
            ...
            $viewAttrs = &$this->patient_name_visits->ViewAttrs;
            $cellAttrs = &$this->patient_name_visits->CellAttrs;
            $hrefValue = &$this->patient_name_visits->HrefValue;
            $linkAttrs = &$this->patient_name_visits->LinkAttrs;
            $this->cellRendered($this->patient_name_visits, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // payment_method
            $currentValue = $this->payment_method->CurrentValue;
            $viewValue = &$this->payment_method->ViewValue;
            $viewAttrs = &$this->payment_method->ViewAttrs;
            $cellAttrs = &$this->payment_method->CellAttrs;
            $hrefValue = &$this->payment_method->HrefValue;
            $linkAttrs = &$this->payment_method->LinkAttrs;
            $this->cellRendered($this->payment_method, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // company
            $currentValue = $this->company->CurrentValue;
            $viewValue = &$this->company->ViewValue;
            $viewAttrs = &$this->company->ViewAttrs;
            $cellAttrs = &$this->company->CellAttrs;
            $hrefValue = &$this->company->HrefValue;
            $linkAttrs = &$this->company->LinkAttrs;
            $this->cellRendered($this->company, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fVisits_Monthsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fVisits_Monthsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
