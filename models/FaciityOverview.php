 ...
 * Page class
 */
class FaciityOverview extends ReportTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FaciityOverview";
    ...
    public $CurrentPageName = "faciityoverview";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Faciity_Overview';
        $this->TableName = 'Faciity Overview';
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Faciity Overview');
        }
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("IncomeReportSummary");
                $Page->run();
                $Page->IncomebyMonth->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->IncomebyMonth->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("ExpensesReportSummary");
                $Page->run();
                $Page->ExpensesbyMonth->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->ExpensesbyMonth->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
