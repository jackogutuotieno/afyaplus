 ...
 * Page class
 */
class PatientsRadiologyReportsView extends PatientsRadiologyReports
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientsRadiologyReportsView";
    ...
    // CSS class/style
    public $CurrentPageName = "patientsradiologyreportsview";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "patientsradiologyreportslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_patients_radiology_reports_details");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("patients_radiology_reports_details", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("patientsradiologyreportsdetailslist?" . Config("TABLE_SHOW_MASTER") . "=patients_radiology_reports&" . GetForeignKeyUrl("fk_radiology_requests_id", $this->radiology_requests_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("PatientsRadiologyReportsDetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'patients_radiology_reports')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=patients_radiology_reports_details"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($item->Visible) {
            ...
            $detailTableLink .= "patients_radiology_reports_details";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fpatients_radiology_reportsview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fpatients_radiology_reportsview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fpatients_radiology_reportsview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (patients_radiology_reports_details)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("patients_radiology_reports_details", explode(",", $this->getCurrentDetailTable()))) {
            $patients_radiology_reports_details = new PatientsRadiologyReportsDetailsList();
            $rsdetail = $patients_radiology_reports_details->loadRs($patients_radiology_reports_details->getDetailFilterFromSession(), $patients_radiology_reports_details->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($patients_radiology_reports_details);
                    $patients_radiology_reports_details->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("patients_radiology_reports_details", $detailTblVar)) {
                $detailPageObj = Container("PatientsRadiologyReportsDetailsGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->radiology_requests_id->IsDetailKey = true;
                    $detailPageObj->radiology_requests_id->CurrentValue = $this->radiology_requests_id->CurrentValue;
                    $detailPageObj->radiology_requests_id->setSessionValue($detailPageObj->radiology_requests_id->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("patientsradiologyreportslist"), "", $this->TableVar, true);
        $pageId = "view";
