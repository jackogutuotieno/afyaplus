...
/**
 * Table class for patient_queue
 */
class PatientQueue extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "patient_queue";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->section->setSelectMultiple(false); // Select one
        $this->section->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->section->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->section->Lookup = new Lookup($this->section, 'patient_queue', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->section->OptionCount = 5;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "patient_queue";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("patientqueuelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "patientqueueview" => $Language->phrase("View"),
            "patientqueueedit" => $Language->phrase("Edit"),
            "patientqueueadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PatientQueueView",
            Config("API_ADD_ACTION") => "PatientQueueAdd",
            Config("API_EDIT_ACTION") => "PatientQueueEdit",
            Config("API_DELETE_ACTION") => "PatientQueueDelete",
            Config("API_LIST_ACTION") => "PatientQueueList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("patientqueueview", $parm);
        } else {
            $url = $this->keyUrl("patientqueueview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientqueueedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientqueuelist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientqueueadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("patientqueuelist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("patientqueuedelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'patient_queue');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'patient_queue', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'patient_queue', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'patient_queue', $fldname, $key, $oldvalue);
            }
        }
    }
