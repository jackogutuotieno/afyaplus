...
/**
 * Table class for lab_test_requests
 */
class LabTestRequests extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->patient_id->Lookup = new Lookup($this->patient_id, 'patients', true, 'id', ["patient_name","","",""], '', '', [], ["x_visit_id"], [], [], [], [], false, '', '', "CONCAT(first_name,' ',last_name)");
        ...
        $this->created_by_user_id->UseFilter = true; // Table header filter
        $this->created_by_user_id->Lookup = new Lookup($this->created_by_user_id, 'users', true, 'id', ["full_name","","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(first_name,' ',last_name)");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "lab_test_requests_details") {
            $detailUrl = Container("lab_test_requests_details")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "labtestrequestslist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "lab_test_requests";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("labtestrequestslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "labtestrequestsview" => $Language->phrase("View"),
            "labtestrequestsedit" => $Language->phrase("Edit"),
            "labtestrequestsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "LabTestRequestsView",
            Config("API_ADD_ACTION") => "LabTestRequestsAdd",
            Config("API_EDIT_ACTION") => "LabTestRequestsEdit",
            Config("API_DELETE_ACTION") => "LabTestRequestsDelete",
            Config("API_LIST_ACTION") => "LabTestRequestsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("labtestrequestsview", $parm);
        } else {
            $url = $this->keyUrl("labtestrequestsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "labtestrequestsadd?" . $parm;
        } else {
            $url = "labtestrequestsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("labtestrequestsedit", $parm);
        } else {
            $url = $this->keyUrl("labtestrequestsedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("labtestrequestslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("labtestrequestsadd", $parm);
        } else {
            $url = $this->keyUrl("labtestrequestsadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("labtestrequestslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("labtestrequestsdelete", $parm);
        }
    }
    ...
    {
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM lab_test_requests";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'lab_test_requests');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'lab_test_requests', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'lab_test_requests', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'lab_test_requests', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowRendered()
    {
        ...
        if ($this->date_created->CurrentValue > $current_date) {
            ...
            $this->status->ViewValue = "New Request"; 
