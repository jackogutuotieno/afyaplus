 ...
 * Page class
 */
class VisitsReportList extends VisitsReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VisitsReportList";
    ...
    public $FormName = "fvisits_reportlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "visitsreportlist";
    ...
    public function setVisibility()
    {
        ...
        $this->visit_month->setVisibility();
    }
    ...
    public function __construct()
    {
        ...
        // Table object (visits_report)
        if (!isset($GLOBALS["visits_report"]) || $GLOBALS["visits_report"]::class == PROJECT_NAMESPACE . "visits_report") {
            $GLOBALS["visits_report"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "visitsreportadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "visitsreportdelete";
        $this->MultiUpdateUrl = "visitsreportupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'visits_report');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "visitsreportview"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fvisits_reportgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fvisits_reportsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->patient_name_visits->AdvancedSearch->toJson(), ","); // Field patient_name_visits
        ...
        $filterList = Concat($filterList, $this->visit_month->AdvancedSearch->toJson(), ","); // Field visit_month
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fvisits_reportsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field patient_name_visits
        $this->patient_name_visits->AdvancedSearch->SearchValue = @$filter["x_patient_name_visits"];
        $this->patient_name_visits->AdvancedSearch->SearchOperator = @$filter["z_patient_name_visits"];
        $this->patient_name_visits->AdvancedSearch->SearchCondition = @$filter["v_patient_name_visits"];
        $this->patient_name_visits->AdvancedSearch->SearchValue2 = @$filter["y_patient_name_visits"];
        $this->patient_name_visits->AdvancedSearch->SearchOperator2 = @$filter["w_patient_name_visits"];
        $this->patient_name_visits->AdvancedSearch->save();
        ...
        // Field visit_month
        $this->visit_month->AdvancedSearch->SearchValue = @$filter["x_visit_month"];
        $this->visit_month->AdvancedSearch->SearchOperator = @$filter["z_visit_month"];
        $this->visit_month->AdvancedSearch->SearchCondition = @$filter["v_visit_month"];
        $this->visit_month->AdvancedSearch->SearchValue2 = @$filter["y_visit_month"];
        $this->visit_month->AdvancedSearch->SearchOperator2 = @$filter["w_visit_month"];
        $this->visit_month->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->patient_name_visits;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->visit_month); // visit_month
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fvisits_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fvisits_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "patient_name_visits");
            ...
            $this->createColumnOption($option, "visit_month");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fvisits_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fvisits_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fvisits_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_visits_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_visits_report",
    ...
    protected function newRow()
    {
        ...
        $row['patient_name_visits'] = $this->patient_name_visits->DefaultValue;
        ...
        $row['visit_month'] = $this->visit_month->DefaultValue;
        return $row;
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fvisits_reportlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fvisits_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
