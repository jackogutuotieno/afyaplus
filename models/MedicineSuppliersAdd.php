 ...
 * Page class
 */
class MedicineSuppliersAdd extends MedicineSuppliers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MedicineSuppliersAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "medicinesuppliersadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("medicinesupplierslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "medicinesupplierslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "medicinesuppliersview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "medicinesupplierslist") {
                            ...
                            $returnUrl = "medicinesupplierslist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'supplier_name' first before field var 'x_supplier_name'
        $val = $CurrentForm->hasValue("supplier_name") ? $CurrentForm->getValue("supplier_name") : $CurrentForm->getValue("x_supplier_name");
        if (!$this->supplier_name->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->supplier_name->Visible = false; // Disable update for API request
            } else {
                $this->supplier_name->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // supplier_name
        $this->supplier_name->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->supplier_name->EditValue = HtmlEncode($this->supplier_name->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->supplier_name->Visible && $this->supplier_name->Required) {
                if (!$this->supplier_name->IsDetailKey && EmptyValue($this->supplier_name->FormValue)) {
                    $this->supplier_name->addErrorMessage(str_replace("%s", $this->supplier_name->caption(), $this->supplier_name->RequiredErrorMessage));
                }
            }
    ...
    {
        if (isset($row['supplier_name'])) { // supplier_name
            $this->supplier_name->setFormValue($row['supplier_name']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("medicinesupplierslist"), "", $this->TableVar, true);
