...
class PatientAdmissionsAdd extends PatientAdmissions
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_ipd_services", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientIpdPrescriptionsGrid");
        if (in_array("patient_ipd_prescriptions", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MedicineIpdDispensationGrid");
        if (in_array("medicine_ipd_dispensation", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("PatientIpdServicesGrid");
            if (in_array("patient_ipd_services", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->admission_id->setSessionValue($this->id->CurrentValue); // Set master key
                $detailPage->patient_id->setSessionValue($this->patient_id->CurrentValue); // Set master key
            ...
            $detailPage = Container("PatientIpdPrescriptionsGrid");
            if (in_array("patient_ipd_prescriptions", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->admission_id->setSessionValue($this->id->CurrentValue); // Set master key
                $detailPage->patient_id->setSessionValue($this->patient_id->CurrentValue); // Set master key
            ...
            $detailPage = Container("MedicineIpdDispensationGrid");
            if (in_array("medicine_ipd_dispensation", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->admission_id->setSessionValue($this->id->CurrentValue); // Set master key
                $detailPage->patient_id->setSessionValue($this->patient_id->CurrentValue); // Set master key
