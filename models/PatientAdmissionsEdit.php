 ...
 * Page class
 */
class PatientAdmissionsEdit extends PatientAdmissions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientAdmissionsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "patientadmissionsedit";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("issue_items", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        ...
        if (in_array("patient_ipd_vitals", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientIpdServicesGrid");
        if (in_array("patient_ipd_services", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientIpdPrescriptionsGrid");
        if (in_array("patient_ipd_prescriptions", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MedicineIpdDispensationGrid");
        if (in_array("medicine_ipd_dispensation", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailPage = Container("PatientIpdVitalsGrid");
            if (in_array("patient_ipd_vitals", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "patient_ipd_vitals"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("PatientIpdServicesGrid");
            if (in_array("patient_ipd_services", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "patient_ipd_services"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("PatientIpdPrescriptionsGrid");
            if (in_array("patient_ipd_prescriptions", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "patient_ipd_prescriptions"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("MedicineIpdDispensationGrid");
            if (in_array("medicine_ipd_dispensation", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "medicine_ipd_dispensation"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
