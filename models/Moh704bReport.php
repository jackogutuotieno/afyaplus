...
/**
 * Table class for moh704b_report
 */
class Moh704bReport extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "moh704b_report";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->gender->Lookup = new Lookup($this->gender, 'moh704b_report', true, 'gender', ["gender","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->countyName->UseFilter = true; // Table header filter
        $this->countyName->Lookup = new Lookup($this->countyName, 'moh704b_report', true, 'countyName', ["countyName","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->subCounty->UseFilter = true; // Table header filter
        $this->subCounty->Lookup = new Lookup($this->subCounty, 'moh704b_report', true, 'subCounty', ["subCounty","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        // registrration_date
        $this->registrration_date = new DbField(
            $this, // Table
            'x_registrration_date', // Variable name
            'registrration_date', // Name
            '`registrration_date`', // Expression
            CastDateFieldForLike("`registrration_date`", 7, "DB"), // Basic search expression
            135, // Type
            76, // Size
            7, // Date/Time format
            false, // Is upload field
            '`registrration_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->registrration_date->InputTextType = "text";
        $this->registrration_date->Raw = true;
        $this->registrration_date->Nullable = false; // NOT NULL field
        $this->registrration_date->Required = true; // Required field
        $this->registrration_date->DefaultErrorMessage = str_replace("%s", DateFormat(7), $Language->phrase("IncorrectDate"));
        $this->registrration_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['registrration_date'] = &$this->registrration_date;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "moh704b_report";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->registrration_date->DbValue = $row['registrration_date'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("moh704breportlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "moh704breportview" => $Language->phrase("View"),
            "moh704breportedit" => $Language->phrase("Edit"),
            "moh704breportadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "Moh704bReportView",
            Config("API_ADD_ACTION") => "Moh704bReportAdd",
            Config("API_EDIT_ACTION") => "Moh704bReportEdit",
            Config("API_DELETE_ACTION") => "Moh704bReportDelete",
            Config("API_LIST_ACTION") => "Moh704bReportList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("moh704breportview", $parm);
        } else {
            $url = $this->keyUrl("moh704breportview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "moh704breportadd?" . $parm;
        } else {
            $url = "moh704breportadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("moh704breportedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("moh704breportlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("moh704breportadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("moh704breportlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("moh704breportdelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->registrration_date->EditValue = FormatDateTime($this->registrration_date->CurrentValue, $this->registrration_date->formatPattern());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->registrration_date);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->patient_name);
                    $doc->exportCaption($this->date_of_birth);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->registrration_date);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->patient_name);
                        $doc->exportField($this->date_of_birth);
