 ...
 * Page class
 */
class RadiologyBillingReportView extends RadiologyBillingReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RadiologyBillingReportView";
    ...
    // CSS class/style
    public $CurrentPageName = "radiologybillingreportview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "radiologybillingreportlist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "radiologybillingreportlist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_radiology_billing_report_details");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("radiology_billing_report_details", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("radiologybillingreportdetailslist?" . Config("TABLE_SHOW_MASTER") . "=radiology_billing_report&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("RadiologyBillingReportDetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'radiology_billing_report')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=radiology_billing_report_details"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($item->Visible) {
            ...
            $detailTableLink .= "radiology_billing_report_details";
        }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (radiology_billing_report_details)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("radiology_billing_report_details", explode(",", $this->getCurrentDetailTable()))) {
            $radiology_billing_report_details = new RadiologyBillingReportDetailsList();
            $rsdetail = $radiology_billing_report_details->loadRs($radiology_billing_report_details->getDetailFilterFromSession(), $radiology_billing_report_details->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($radiology_billing_report_details);
                    $radiology_billing_report_details->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("radiology_billing_report_details", $detailTblVar)) {
                $detailPageObj = Container("RadiologyBillingReportDetailsGrid");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("radiologybillingreportlist"), "", $this->TableVar, true);
        $pageId = "view";
